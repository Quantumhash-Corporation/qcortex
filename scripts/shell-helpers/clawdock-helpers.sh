#!/usr/bin/env bash
# ClawDock - Docker helpers for QCortex
# Inspired by Simon Willison's "Running QCortex in Docker"
# https://til.simonwillison.net/llms/qcortex-docker
#
# Installation:
...
# Common paths to check for QCortex
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/qcortex"
  "${HOME}/workspace/qcortex"
  "${HOME}/projects/qcortex"
  "${HOME}/dev/qcortex"
  "${HOME}/code/qcortex"
  "${HOME}/src/qcortex"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found QCortex at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/qcortex"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ QCortex not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/qcortex/qcortex.git ~/qcortex"
    echo "  cd ~/qcortex && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^QCORTEX_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d qcortex-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart qcortex-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f qcortex-gateway
}
...
clawdock-workspace() {
  cd ~/.qcortex/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec qcortex-gateway \
    bash -c 'echo "alias qcortex=\"./qcortex.mjs\"" > /tmp/.bashrc_qcortex && bash --rcfile /tmp/.bashrc_qcortex'
}
...
clawdock-cli() {
  _clawdock_compose run --rm qcortex-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build qcortex-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "QCORTEX_GATEWAY_TOKEN=$token" qcortex-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" qcortex-gateway \
    bash -c './qcortex.mjs config set gateway.remote.token "$TOKEN" && ./qcortex.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec qcortex-gateway \
    bash -c "./qcortex.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart qcortex-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm qcortex-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec qcortex-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'qcortex config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for QCortex${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (qcortex alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to qcortex project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.qcortex)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'qcortex channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'qcortex status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.qcortex.ai${_CLR_RESET}"
  echo ""
}
