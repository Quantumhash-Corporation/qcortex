...
INSTALL_URL="${QCORTEX_INSTALL_URL:-https://qcortex.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${QCORTEX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${QCORTEX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="qcortex"
PACKAGE_NAME="${QCORTEX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (QCORTEX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${QCORTEX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${QCORTEX_INSTALL_LATEST_OUT:-}"
fi
