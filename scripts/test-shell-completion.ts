 ...
 * `qcortex update`. Use it to verify the completion installation flow
...
\`qcortex update\`, \`qcortex doctor\`, and \`qcortex onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# QCortex Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`qcortex completion --install\` later to enable.`));
    return;
  }
