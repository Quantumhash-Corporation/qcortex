...
#
# Suggested cron: */30 * * * * /home/admin/qcortex/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send QCortex notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., qcortex-alerts)
...
STATE_FILE="$HOME/.qcortex/auth-monitor-state"
...
send_notification() {
    ...
    # Send via QCortex if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use qcortex
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via QCortex to $NOTIFY_PHONE..."
            qcortex send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! QCortex is down. Run: ssh l36 '~/qcortex/scripts/mobile-reauth.sh'" "urgent"
    exit 1
