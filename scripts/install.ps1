# QCortex Installer for Windows (PowerShell)
# Usage: iwr -useb https://qcortex.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://qcortex.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\qcortex",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Install-QCortexNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing QCortex (qcortex@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g qcortex@$Version --no-fund --no-audit 2>&1
        Write-Host "QCortex installed" -Level success
        return $true
    } catch {
...
function Install-QCortexGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing QCortex from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/qcortex/qcortex.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\qcortex\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\qcortex.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install QCortex from git to $GitDir" -Level info
        } else {
            Install-QCortexGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install QCortex via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-QCortexNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'qcortex onboard' to complete setup" -Level info
    }
