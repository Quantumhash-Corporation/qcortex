...
IMAGE_NAME="qcortex-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="qcortex-net-e2e-$$"
GW_NAME="qcortex-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "QCORTEX_GATEWAY_TOKEN=$TOKEN" \
  -e "QCORTEX_SKIP_CHANNELS=1" \
  -e "QCORTEX_SKIP_GMAIL_WATCHER=1" \
  -e "QCORTEX_SKIP_CRON=1" \
  -e "QCORTEX_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
