...
type DriverMode = "token" | "webhook" | "qcortex";
...
function resolveStateDir(): string {
  const override = process.env.QCORTEX_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.QCORTEX_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".qcortex");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver qcortex] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) QCortex spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|qcortex> Driver transport mode (default: token)\n" +
    ...
    "  --qcortex-bin <path>        QCortex CLI binary for driver=qcortex (default: qcortex)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  QCORTEX_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  QCORTEX_DISCORD_SMOKE_DRIVER\n" +
    "  QCORTEX_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  QCORTEX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  QCORTEX_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  QCORTEX_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  QCORTEX_DISCORD_SMOKE_AGENT\n" +
    "  QCORTEX_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  QCORTEX_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  QCORTEX_DISCORD_SMOKE_POLL_MS\n" +
    "  QCORTEX_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  QCORTEX_DISCORD_SMOKE_QCORTEX_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.QCORTEX_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.QCORTEX_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.QCORTEX_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.QCORTEX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.QCORTEX_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.QCORTEX_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.QCORTEX_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.QCORTEX_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.QCORTEX_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.QCORTEX_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.QCORTEX_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.QCORTEX_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const qcortexBin =
    resolveArg("--qcortex-bin") || process.env.QCORTEX_DISCORD_SMOKE_QCORTEX_BIN || "qcortex";
...
async function qcortexCliJson<T>(params: { qcortexBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.qcortexBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`qcortex ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await qcortexCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    qcortexBin: params.qcortexBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "qcortex") {
    return await readMessagesWithOpenclaw({
      qcortexBin: params.args.qcortexBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `qcortex-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await qcortexCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("qcortex message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from QCortex.`,
        diagnostics: {
          bindingCandidates: [
            {
