...
# QCortex Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://qcortex.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one QCortex."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${QCORTEX_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 QCortex Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://qcortex.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$QCORTEX_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.qcortex.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_qcortex_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.qcortex-* "$npm_root"/qcortex 2>/dev/null || true
}

extract_qcortex_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_qcortex_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/qcortex" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/qcortex"|"/usr/local/bin/qcortex")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/qcortex/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale qcortex symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing qcortex binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing QCortex package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_qcortex_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "QCortex npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*qcortex" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_qcortex_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_qcortex_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "QCortex npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an qcortex binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "QCORTEX_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "QCORTEX_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "QCORTEX_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "QCORTEX_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "QCORTEX_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "QCORTEX_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "QCORTEX_BETA" "CLAWDBOT_BETA"
map_legacy_env "QCORTEX_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "QCORTEX_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "QCORTEX_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "QCORTEX_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "QCORTEX_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "QCORTEX_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${QCORTEX_TAGLINE_INDEX:-}" ]]; then
        if [[ "${QCORTEX_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((QCORTEX_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${QCORTEX_NO_ONBOARD:-0}
NO_PROMPT=${QCORTEX_NO_PROMPT:-0}
DRY_RUN=${QCORTEX_DRY_RUN:-0}
INSTALL_METHOD=${QCORTEX_INSTALL_METHOD:-}
QCORTEX_VERSION=${QCORTEX_VERSION:-latest}
USE_BETA=${QCORTEX_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/qcortex"
GIT_DIR=${QCORTEX_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${QCORTEX_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${QCORTEX_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${QCORTEX_VERBOSE:-0}"
QCORTEX_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
QCortex installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://qcortex.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/qcortex)
...
Environment variables:
  QCORTEX_INSTALL_METHOD=git|npm
  QCORTEX_VERSION=latest|next|<semver>
  QCORTEX_BETA=0|1
  QCORTEX_GIT_DIR=...
  QCORTEX_GIT_UPDATE=0|1
  QCORTEX_NO_PROMPT=1
  QCORTEX_DRY_RUN=1
  QCORTEX_NO_ONBOARD=1
  QCORTEX_VERBOSE=1
  QCORTEX_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://qcortex.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://qcortex.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://qcortex.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected QCortex checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a QCortex source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_qcortex_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"qcortex"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://qcortex.ai/install.sh | bash"
}
...
ensure_qcortex_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/qcortex" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/qcortex" ]]; then
        ln -sf "$npm_root/qcortex/dist/entry.js" "${npm_bin}/qcortex"
        ui_info "Created qcortex bin link at ${npm_bin}/qcortex"
    fi
    return 0
}

# Check for existing QCortex installation
check_existing_qcortex() {
    if [[ -n "$(type -P qcortex 2>/dev/null || true)" ]]; then
        ui_info "Existing QCortex installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make qcortex show as \"command not found\" in new terminals."
...
warn_qcortex_not_found() {
    ui_warn "Installed, but qcortex is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t qcortex 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named qcortex; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_qcortex_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P qcortex 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/qcortex" ]]; then
        echo "${npm_bin}/qcortex"
        return 0
    fi
...
install_qcortex_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/qcortex/qcortex.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing QCortex from git checkout: ${repo_dir}"
    else
        ui_info "Installing QCortex from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning QCortex" git clone "$repo_url" "$repo_dir"
    fi
    ...
    # Rebranding: OpenClaw -> QCortex
    ui_info "Applying QCortex rebranding..."
    find "$repo_dir" -type f \
      ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/dist/*" \
      ! -name "pnpm-lock.yaml" ! -name "*.png" ! -name "*.jpg" ! -name "*.ico" \
      ! -name "*.zip" ! -name "*.tar" ! -name "*.gz" -print0 | \
      while IFS= read -r -d \'\' file; do
        file "$file" | grep -q "text" && \
          sed -i \'\'  \'s/OpenClaw/QCortex/g; s/openclaw/qcortex/g; s/OPENCLAW/QCORTEX/g\' "$file" || true
      done
    [ -f "$repo_dir/openclaw.mjs" ] && mv "$repo_dir/openclaw.mjs" "$repo_dir/qcortex.mjs" || true
    find "$repo_dir/src" -name "*openclaw*" -type f | while read f; do
      mv "$f" "$(echo "$f" | sed \'s/openclaw/qcortex/g\')" 2>/dev/null || true
    done
    [ -d "$repo_dir/apps/shared/OpenClawKit" ] && mv "$repo_dir/apps/shared/OpenClawKit" "$repo_dir/apps/shared/QCortexKit" || true
    find "$repo_dir/apps/shared/QCortexKit/Sources" -type d -name "OpenClaw*" 2>/dev/null | while read d; do
      mv "$d" "$(echo "$d" | sed \'s/OpenClaw/QCortex/g\')" || true
    done
    sed -i \'\'  \'s/"name": "openclaw"/"name": "qcortex"/\' "$repo_dir/package.json" || true
    find "$repo_dir/extensions" -name "openclaw.plugin.json" | while read f; do
      mv "$f" "$(dirname "$f")/qcortex.plugin.json"
    done || true
    ui_success "Rebranding applied"
    ...
    run_quiet_step "Building QCortex" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/qcortex" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/qcortex"
    ui_success "QCortex wrapper installed to \$HOME/.local/bin/qcortex"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view qcortex dist-tags.beta 2>/dev/null || true)"
...
install_qcortex() {
    local package_name="qcortex"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            QCORTEX_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="qcortex"
        else
            QCORTEX_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${QCORTEX_VERSION}" ]]; then
        QCORTEX_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${QCORTEX_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing QCortex v${resolved_version}"
    else
        ui_info "Installing QCortex (${QCORTEX_VERSION})"
    fi
    local install_spec=""
    if [[ "${QCORTEX_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${QCORTEX_VERSION}"
    fi

    if ! install_qcortex_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_qcortex_paths
        install_qcortex_npm "${install_spec}"
    fi

    if [[ "${QCORTEX_VERSION}" == "latest" && "${package_name}" == "qcortex" ]]; then
        if ! resolve_qcortex_bin &> /dev/null; then
            ui_warn "npm install qcortex@latest failed; retrying qcortex@next"
            cleanup_npm_qcortex_paths
            install_qcortex_npm "qcortex@next"
        fi
    fi

    ensure_qcortex_bin_link || true

    ui_success "QCortex installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${QCORTEX_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_qcortex_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (qcortex not on PATH yet)"
        warn_qcortex_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${QCORTEX_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.qcortex/workspace-${profile}"
    else
        echo "${HOME}/.qcortex/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${QCORTEX_CONFIG_PATH:-$HOME/.qcortex/qcortex.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run qcortex onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but qcortex not on PATH; skipping onboarding"
        warn_qcortex_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run qcortex onboard to retry"
        return
    }
}

resolve_qcortex_version() {
    local version=""
    local claw="${QCORTEX_BIN:-}"
    if [[ -z "$claw" ]] && command -v qcortex &> /dev/null; then
        claw="$(command -v qcortex)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/qcortex/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/qcortex/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_qcortex_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found QCortex checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set QCORTEX_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_qcortex; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing QCortex"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g qcortex &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g qcortex 2>/dev/null || true
        ...
        install_qcortex_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/qcortex" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/qcortex"
    ...
    QCORTEX_BIN="$(resolve_qcortex_bin || true)"
    ...
    installed_version=$(resolve_qcortex_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 QCortex installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 QCortex installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/qcortex"
        ui_kv "Update command" "qcortex update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://qcortex.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running qcortex doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                QCORTEX_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                QCORTEX_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                QCORTEX_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run qcortex doctor and qcortex plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run qcortex onboard later"
        else
            local config_path="${QCORTEX_CONFIG_PATH:-$HOME/.qcortex/qcortex.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (qcortex not on PATH yet)"
                    warn_qcortex_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run qcortex onboard to finish setup"
            return 0
        fi
    fi
    ...
    if command -v qcortex &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (qcortex daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if QCORTEX_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: qcortex daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${QCORTEX_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
