...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/qcortex-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// QCORTEX_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.QCORTEX_TEST_VM_FORKS === "1" ||
  (process.env.QCORTEX_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks && !lowMemLocalHost);
const disableIsolation = process.env.QCORTEX_TEST_NO_ISOLATE === "1";
const includeGatewaySuite = process.env.QCORTEX_TEST_INCLUDE_GATEWAY === "1";
const includeExtensionsSuite = process.env.QCORTEX_TEST_INCLUDE_EXTENSIONS === "1";
const rawTestProfile = process.env.QCORTEX_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const shardOverride = Number.parseInt(process.env.QCORTEX_TEST_SHARDS ?? "", 10);
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.QCORTEX_TEST_SHARD_INDEX ?? "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
})();

if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] QCORTEX_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires QCORTEX_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.QCORTEX_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
