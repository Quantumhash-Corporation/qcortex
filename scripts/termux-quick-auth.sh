...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/qcortex/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "QCortex Re-Auth" -c "After getting key, run: ssh $SERVER '~/qcortex/scripts/mobile-reauth.sh'" --id qcortex-auth
        ;;
    *)
