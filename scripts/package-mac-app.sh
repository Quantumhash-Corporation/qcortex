...
# Build and bundle QCortex into a minimal .app we can open.
# Outputs to dist/QCortex.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/QCortex.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="QCortex"
BUNDLE_ID="${BUNDLE_ID:-ai.qcortex.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/qcortex/qcortex/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/QCortex/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :QCortexBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :QCortexGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/QCortex"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/QCortex"
fi
chmod +x "$APP_ROOT/Contents/MacOS/QCortex"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/QCortex" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/QCortex/Resources/QCortex.icns" "$APP_ROOT/Contents/Resources/QCortex.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/QCortex/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying QCortexKit resources"
QCORTEXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/QCortexKit_QCortexKit.bundle"
if [ -d "$QCORTEXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/QCortexKit_QCortexKit.bundle"
  cp -R "$QCORTEXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/QCortexKit_QCortexKit.bundle"
else
  echo "WARN: QCortexKit resource bundle not found at $QCORTEXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running QCortex"
killall -q QCortex 2>/dev/null || true
