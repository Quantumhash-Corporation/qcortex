...
const shouldBuild = (deps) => {
  if (deps.env.QCORTEX_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.QCORTEX_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[qcortex] ${message}\n`);
};

const runQCortex = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["qcortex.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
