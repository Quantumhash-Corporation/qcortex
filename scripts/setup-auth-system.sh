#!/bin/bash
# Setup QCortex Auth Management System
...
echo "=== QCortex Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'qcortex-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  QCortex message: Send warning via QCortex itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/qcortex-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/qcortex-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/qcortex-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now qcortex-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status qcortex-auth-monitor.timer"
echo ""
echo "Quick commands:"
