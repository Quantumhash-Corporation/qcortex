...
- `/usage cost` in chats: local cost summary aggregated from QCortex session logs.
- CLI: `qcortex status --usage` prints a full per-provider breakdown.
- CLI: `qcortex channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
