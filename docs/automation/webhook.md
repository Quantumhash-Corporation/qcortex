...
read_when:
  ...
  - Wiring external systems into QCortex
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your QCortex config directory (typically `~/.qcortex/hooks/transforms`).
...
- `qcortex webhooks gmail setup` writes `hooks.gmail` config for `qcortex webhooks gmail run`.
