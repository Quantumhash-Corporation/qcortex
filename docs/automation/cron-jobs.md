...
- Jobs persist under `~/.qcortex/cron/` so restarts don’t lose schedules.
...
qcortex cron list
qcortex cron run <job-id>
qcortex cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.qcortex/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `qcortex cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, QCortex applies a
...
If `delivery` is omitted for isolated jobs, QCortex defaults to `announce`.
...
- CLI equivalents: `qcortex cron add --light-context ...` and `qcortex cron edit --light-context`.
...
- Job store: `~/.qcortex/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.qcortex/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, QCortex classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, QCortex archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `qcortex cron runs` and adjust retention before logs become large
...
qcortex cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
qcortex cron edit <jobId> --agent ops
qcortex cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
qcortex cron run <jobId>
qcortex cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
qcortex cron edit <jobId> \
...
```bash
qcortex system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`qcortex system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `QCORTEX_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- QCortex applies exponential retry backoff for recurring jobs after consecutive errors:
