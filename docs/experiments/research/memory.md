...
read_when:
  - Designing workspace memory (~/.qcortex/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep QCortex integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.qcortex/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.qcortex/workspace` as canonical human-readable memory.
...
```
~/.qcortex/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in QCortex**, but keep a separable core library.

### Why integrate into QCortex?

- QCortex already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `qcortex memory recall "…" --k 25 --since 30d`
  - `qcortex memory reflect --since 7d`
...
Pragmatic take for `~/.qcortex/workspace`:
