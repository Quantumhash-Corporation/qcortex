...
This plan defines how QCortex should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- QCortex core owns ACP control plane concerns
...
QCortex should not reimplement ACP transport internals in core.
QCortex should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in QCortex, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under QCortex state dir
...
- ACP event stream is normalized to QCortex reply chunks
...
For the first production backend (`extensions/acpx`), QCortex and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses QCortex `sessionKey`)
...
- QCortex consumes ndjson events from `acpx --format json --json-strict`
