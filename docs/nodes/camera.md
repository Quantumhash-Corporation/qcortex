...
QCortex supports **camera capture** for agent workflows:
...
```bash
qcortex nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
qcortex nodes camera snap --node <id> --facing front
qcortex nodes camera clip --node <id> --duration 3000
qcortex nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`qcortex.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `qcortex` CLI to invoke camera commands on the macOS node.

Examples:

```bash
qcortex nodes camera list --node <id>            # list camera ids
qcortex nodes camera snap --node <id>            # prints MEDIA:<path>
qcortex nodes camera snap --node <id> --max-width 1280
qcortex nodes camera snap --node <id> --delay-ms 2000
qcortex nodes camera snap --node <id> --device-id <id>
qcortex nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
qcortex nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
qcortex nodes camera clip --node <id> --device-id <id>
qcortex nodes camera clip --node <id> --no-audio
```

Notes:

- `qcortex nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
qcortex nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
