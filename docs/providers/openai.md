---
summary: "Use OpenAI via API keys or Codex subscription in QCortex"
read_when:
  - You want to use OpenAI models in QCortex
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like QCortex.
...
```bash
qcortex onboard --auth-choice openai-api-key
# or non-interactive
qcortex onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. QCortex forwards both through the `openai/*` Responses path.
...
qcortex onboard --auth-choice openai-codex

# Or run OAuth directly
qcortex models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. QCortex
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

QCortex uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), QCortex also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. QCortex enables it by default for
...
QCortex, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), QCortex now auto-enables OpenAI server-side
