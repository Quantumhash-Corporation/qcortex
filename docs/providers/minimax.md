---
summary: "Use MiniMax M2.5 in QCortex"
read_when:
  - You want MiniMax models in QCortex
...
- **Compatibility:** QCortex still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
qcortex plugins enable minimax-portal-auth  # skip if already loaded.
qcortex gateway restart  # restart if gateway is already running
qcortex onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/qcortex/qcortex/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `qcortex.json`:
...
- Use `qcortex models list` and `qcortex models set minimax/MiniMax-M2.5` to switch.
...
- Running `qcortex configure` and selecting **MiniMax M2.5**, or
