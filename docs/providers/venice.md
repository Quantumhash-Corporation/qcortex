---
summary: "Use Venice AI privacy-focused models in QCortex"
read_when:
  - You want privacy-focused inference in QCortex
...
```bash
qcortex onboard --auth-choice venice-api-key
```
...
```bash
qcortex agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, QCortex shows all available Venice models. Pick based on your needs:
...
```bash
qcortex models set venice/kimi-k2-5
qcortex models set venice/claude-opus-4-6
```

List all available models:

```bash
qcortex models list | grep venice
```

## Configure via `qcortex configure`

1. Run `qcortex configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
QCortex automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
qcortex agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
qcortex agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
qcortex agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
qcortex agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
qcortex agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `qcortex models list` to see currently available models. Some models may be temporarily offline.
