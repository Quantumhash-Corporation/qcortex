...
```bash
qcortex tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `qcortex tui --deliver`
...
- Local shell commands receive `QCORTEX_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `qcortex logs --follow`.
- Confirm the agent can run: `qcortex status` and `qcortex models status`.
...
- No agents in picker: check `qcortex agents list` and your routing config.
