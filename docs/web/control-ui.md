...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/qcortex`)
...
If the page fails to load, start the Gateway first: `qcortex gateway`.
...
you revoke it with `qcortex devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.qcortex/qcortex.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop qcortex`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. QCortex
...
QCortex **blocks** Control UI connections without device identity.
