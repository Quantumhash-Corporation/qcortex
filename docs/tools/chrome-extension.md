---
summary: "Chrome extension: let QCortex drive your existing Chrome tab"
read_when:
...
The QCortex Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate qcortex-managed Chrome profile.
...
QCortex then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the QCortex release (npm package) as static files. There is no separate “build” step.

After upgrading QCortex:

- Re-run `qcortex browser extension install` to refresh the installed files under your QCortex state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

QCortex ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `QCORTEX_GATEWAY_TOKEN`)
...
- Open the tab you want QCortex to control.
...
- `ON`: attached; QCortex can drive that tab.
...
Debugging: `qcortex sandbox explain`
...
`qcortex browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `qcortex browser extension install` first to copy the extension to a stable location under your QCortex state directory.
...
- **This is not isolated** like the dedicated qcortex-managed profile.
