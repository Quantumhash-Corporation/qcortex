...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let QCortex run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask QCortex in plain language to "run this in Codex" or "start Claude Code in a thread", QCortex should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want QCortex-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | QCortex native sub-agent runtime   |
...
- QCortex binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, QCortex returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning QCortex agent id.
...
- QCortex ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, QCortex uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, QCortex may default to persistent behavior per runtime path
...
ACP sessions currently run on the host runtime, not inside the QCortex sandbox.
...
If no target resolves, QCortex returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, QCortex returns a clear unsupported-control error.
...
When QCortex uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal QCortex `agentId` path).
...
```bash
qcortex plugins install acpx
qcortex config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
qcortex plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@qcortex/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from QCortex workspace directory.
...
- QCortex startup remains non-blocking while the backend health check runs.
...
```bash
qcortex config set plugins.entries.acpx.config.permissionMode approve-all
qcortex config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** QCortex currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
