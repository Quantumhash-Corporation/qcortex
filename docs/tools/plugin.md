---
summary: "QCortex plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends QCortex with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core QCortex yet (or you want to keep optional features out of your main
install).
...
```bash
qcortex plugins install @qcortex/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@qcortex/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@qcortex/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@qcortex/zalouser`
- [Matrix](/channels/matrix) — `@qcortex/matrix`
- [Nostr](/channels/nostr) — `@qcortex/nostr`
- [Zalo](/channels/zalo) — `@qcortex/zalo`
- [Microsoft Teams](/channels/msteams) — `@qcortex/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

QCortex plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `qcortex/plugin-sdk` import when
authoring plugins:

- `qcortex/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `qcortex/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `qcortex/plugin-sdk/telegram` for Telegram channel plugins.
- `qcortex/plugin-sdk/discord` for Discord channel plugins.
- `qcortex/plugin-sdk/slack` for Slack channel plugins.
- `qcortex/plugin-sdk/signal` for Signal channel plugins.
- `qcortex/plugin-sdk/imessage` for iMessage channel plugins.
- `qcortex/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `qcortex/plugin-sdk/line` for LINE channel plugins.
- `qcortex/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `qcortex/plugin-sdk/acpx`, `qcortex/plugin-sdk/bluebubbles`,
  `qcortex/plugin-sdk/copilot-proxy`, `qcortex/plugin-sdk/device-pair`,
  `qcortex/plugin-sdk/diagnostics-otel`, `qcortex/plugin-sdk/diffs`,
  `qcortex/plugin-sdk/feishu`,
  `qcortex/plugin-sdk/google-gemini-cli-auth`, `qcortex/plugin-sdk/googlechat`,
  `qcortex/plugin-sdk/irc`, `qcortex/plugin-sdk/llm-task`,
  `qcortex/plugin-sdk/lobster`, `qcortex/plugin-sdk/matrix`,
  `qcortex/plugin-sdk/mattermost`, `qcortex/plugin-sdk/memory-core`,
  `qcortex/plugin-sdk/memory-lancedb`,
  `qcortex/plugin-sdk/minimax-portal-auth`,
  `qcortex/plugin-sdk/nextcloud-talk`, `qcortex/plugin-sdk/nostr`,
  `qcortex/plugin-sdk/open-prose`, `qcortex/plugin-sdk/phone-control`,
  `qcortex/plugin-sdk/qwen-portal-auth`, `qcortex/plugin-sdk/synology-chat`,
  `qcortex/plugin-sdk/talk-voice`, `qcortex/plugin-sdk/test-utils`,
  `qcortex/plugin-sdk/thread-ownership`, `qcortex/plugin-sdk/tlon`,
  `qcortex/plugin-sdk/twitch`, `qcortex/plugin-sdk/voice-call`,
  `qcortex/plugin-sdk/zalo`, and `qcortex/plugin-sdk/zalouser`.

Compatibility note:

- `qcortex/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `qcortex status`, `qcortex status --all`,
  `qcortex channels status`, `qcortex channels resolve`, and doctor/config
...
- Set `QCORTEX_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `QCORTEX_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `QCORTEX_PLUGIN_DISCOVERY_CACHE_MS` and
  `QCORTEX_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

QCortex scans, in order:
...
- `<workspace>/.qcortex/extensions/*.ts`
- `<workspace>/.qcortex/extensions/*/index.ts`

3. Global extensions

- `~/.qcortex/extensions/*.ts`
- `~/.qcortex/extensions/*/index.ts`

4. Bundled extensions (shipped with QCortex, mostly disabled by default)

- `<qcortex>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `qcortex plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, QCortex logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. QCortex blocks candidates when:
...
Each plugin must include a `qcortex.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `qcortex.extensions`:
...
Security guardrail: every `qcortex.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `qcortex plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `qcortex.channel` and
install hints via `qcortex.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@qcortex/nextcloud-talk",
  "qcortex": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@qcortex/nextcloud-talk",
...
QCortex can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.qcortex/mpm/plugins.json`
- `~/.qcortex/mpm/catalog.json`
- `~/.qcortex/plugins/catalog.json`

Or point `QCORTEX_PLUGIN_CATALOG_PATHS` (or `QCORTEX_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "qcortex": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, QCortex uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `qcortex.plugin.json` (`configSchema`).
...
QCortex augments `uiHints` at runtime based on discovered plugins:
...
qcortex plugins install <path>                 # copy a local file/dir into ~/.qcortex/extensions/<id>
qcortex plugins install ./extensions/voice-call # relative path ok
qcortex plugins install ./plugin.tgz           # install from a local tarball
qcortex plugins install ./plugin.zip           # install from a local zip
qcortex plugins install -l ./extensions/voice-call # link (no copy) for dev
qcortex plugins install @qcortex/voice-call # install from npm
qcortex plugins install @qcortex/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, QCortex warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `qcortex voicecall`).
...
- Plugin-managed hooks show up in `qcortex hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `qcortex hooks`; enable/disable the plugin instead.
...
- When disabled, QCortex blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside QCortex (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `qcortex models auth login --provider <id> [--method <id>]`
...
- `config`: The current QCortex config
...
- Main package: `qcortex` (this repo)
- Plugins: separate npm packages under `@qcortex/*` (example: `@qcortex/voice-call`)

Publishing contract:

- Plugin `package.json` must include `qcortex.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `qcortex plugins install <npm-spec>` uses `npm pack`, extracts into `~/.qcortex/extensions/<id>/`, and enables it in config.
...
- CLI: `qcortex voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `qcortex.extensions` points at the built entrypoint (`dist/index.js`).
