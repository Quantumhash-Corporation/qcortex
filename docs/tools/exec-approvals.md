...
- **gateway host** → `qcortex` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.qcortex/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.qcortex/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `qcortex approvals allowlist ...`).
- `qcortex security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `qcortex doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.qcortex/exec-approvals.json` directly.

CLI: `qcortex approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
