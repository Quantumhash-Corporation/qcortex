...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `qcortex sandbox explain`.
...
Legacy `agent.*` configs are migrated by `qcortex doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=qcortex-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${QCORTEX_STATE_DIR:-$HOME/.qcortex}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
