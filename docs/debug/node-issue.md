...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in QCortex
...
Running QCortex via Node with `tsx` fails at startup with:

```
[qcortex] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `qcortex status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch qcortex.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node qcortex.mjs status` works on Node 25.
