...
- During heartbeat runs, QCortex treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `qcortex sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, QCortex skips the heartbeat run entirely (no model call).
...
```bash
qcortex system event --text "Check for urgent follow-ups" --mode now
```
