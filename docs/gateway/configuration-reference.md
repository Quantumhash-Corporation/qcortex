...
description: "Complete field-by-field reference for ~/.qcortex/qcortex.json"
summary: "Complete reference for every QCortex config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.qcortex/qcortex.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — QCortex uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.qcortex/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `qcortex doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `qcortex doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["qcortex-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-qcortex",
          requireMention: false,
...
- QCortex additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `qcortex plugins install @qcortex/mattermost`.
...
- `commands.callbackUrl` must resolve to the QCortex gateway endpoint and be reachable from the Mattermost server.
...
QCortex spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `qcortex channels add` (or channel onboarding) while still on a single-account top-level channel config, QCortex moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `qcortex doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@qcortex", "qcortex"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@qcortex"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `qcortex.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, QCortex auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/qcortex" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, QCortex assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "qcortex-sandbox-browser:bookworm-slim",
          network: "qcortex-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.qcortex/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `qcortex.json`.
noVNC observer access uses VNC auth by default and QCortex emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `qcortex-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `QCORTEX_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `QCORTEX_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `QCORTEX_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.qcortex/workspace",
        agentDir: "~/.qcortex/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@qcortex"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.qcortex/workspace-home" },
      { id: "work", workspace: "~/.qcortex/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, QCortex resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.qcortex/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.qcortex/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.qcortex/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.qcortex/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, QCortex starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.qcortex/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, QCortex treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:qcortex`    | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.qcortex/attachments/<uuid>/` with a `.manifest.json`.
...
QCortex uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.qcortex/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `QCORTEX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), QCortex forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `qcortex onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `qcortex onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `qcortex onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `qcortex onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `qcortex onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `qcortex onboard --auth-choice minimax-api`.
...
- Loaded from `~/.qcortex/extensions`, `<workspace>/.qcortex/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `qcortex plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      qcortex: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or QCORTEX_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `QCORTEX_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `QCORTEX_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
```bash
QCORTEX_CONFIG_PATH=~/.qcortex/a.json \
QCORTEX_STATE_DIR=~/.qcortex-a \
qcortex gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.qcortex-dev` + port `19001`), `--profile <name>` (uses `~/.qcortex-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.qcortex/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-qcortex-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `QCORTEX_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.qcortex/workspace/canvas",
    liveReload: true,
    // enabled: false, // or QCORTEX_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__qcortex__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__qcortex__/a2ui/`
...
- Also serves A2UI at `/__qcortex__/a2ui/`.
...
- Hostname defaults to `qcortex`. Override with `QCORTEX_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.qcortex/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `qcortex dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.qcortex/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${QCORTEX_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `qcortex.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.qcortex/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/qcortex-vault-resolver",
...
- Legacy OAuth imports from `~/.qcortex/credentials/oauth.json`.
...
- Default log file: `/tmp/qcortex/qcortex-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one QCortex.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `QCORTEX_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `qcortex doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `qcortex.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
