...
read_when:
  - Running QCortex behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of QCortex
...
- You run QCortex behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. QCortex checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. QCortex extracts the user identity from the configured header
...
- QCortex can stay on loopback HTTP behind the proxy.
...
If QCortex itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://qcortex.example.com
    to: http://qcortex-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`qcortex security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update QCortex config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `qcortex security audit` and review findings
