...
QCortex supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `QCORTEX_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, QCortex runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). QCortex validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/QCortex QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/qcortex"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `qcortex.json` refs)
...
When reload-time activation fails after a healthy state, QCortex enters degraded secrets state.
...
- Strict command paths (for example `qcortex memory` remote-memory paths and `qcortex qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `qcortex status`, `qcortex status --all`, `qcortex channels status`, `qcortex channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `qcortex secrets reload`.
...
```bash
qcortex secrets audit --check
qcortex secrets configure
qcortex secrets audit --check
```
...
- plaintext values at rest (`qcortex.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `qcortex.json` refs)
...
- lets you select supported secret-bearing fields in `qcortex.json` plus `auth-profiles.json` for one agent scope
...
- `qcortex secrets configure --providers-only`
- `qcortex secrets configure --skip-provider-setup`
- `qcortex secrets configure --agent <id>`
...
```bash
qcortex secrets apply --from /tmp/qcortex-secrets-plan.json
qcortex secrets apply --from /tmp/qcortex-secrets-plan.json --dry-run
```
...
QCortex intentionally does not write rollback backups containing historical plaintext secret values.
