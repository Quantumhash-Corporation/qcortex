...
Tlon is a decentralized messenger built on Urbit. QCortex connects to your Urbit ship and can
...
```bash
qcortex plugins install @qcortex/tlon
```

Local checkout (when running from a git repo):

```bash
qcortex plugins install ./extensions/tlon
```
...
By default, QCortex blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `qcortex message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, QCortex replies in-thread.
