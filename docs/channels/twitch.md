...
read_when:
  - Setting up Twitch chat integration for QCortex
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. QCortex connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
qcortex plugins install @qcortex/twitch
```

Local checkout (when running from a git repo):

```bash
qcortex plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `QCORTEX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "qcortex", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use QCORTEX_TWITCH_ACCESS_TOKEN env var)
...
```bash
QCORTEX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
