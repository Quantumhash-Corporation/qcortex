...
read_when:
  - You want QCortex to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables QCortex to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`qcortex onboard`) and `qcortex channels add` list optional channel plugins.
...
```bash
qcortex plugins install @qcortex/nostr
```

Use a local checkout (dev workflows):

```bash
qcortex plugins install --link <path-to-qcortex>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "qcortex@example.com",
        "lud16": "qcortex@example.com"
      }
    }
  }
}
