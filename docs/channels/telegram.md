...
```
    ...
    Telegram does **not** use `qcortex channels login telegram`; configure token in config/env, then start gateway.
...
qcortex pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `qcortex doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `qcortex doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `qcortex logs --follow`
...
- DM messages can carry `message_thread_id`; QCortex routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (native drafts + message edits)">
    QCortex can stream partial replies in real time:
    ...
    - DM: QCortex updates the draft in place (no extra preview message)
    - group/topic: QCortex keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), QCortex falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, QCortex skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, QCortex automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, QCortex retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - QCortex pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.qcortex/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, QCortex enqueues system events like:
...
```bash
qcortex message send --channel telegram --target 123456789 --message "hi"
qcortex message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `qcortex message poll` and support forum topics:

```bash
qcortex message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
qcortex message poll --channel telegram --target -1001234567890:topic:42 \
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `qcortex channels status` warns when config expects unmentioned group messages.
    - `qcortex channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `qcortex logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, QCortex now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `QCORTEX_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `QCORTEX_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `QCORTEX_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `qcortex doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `qcortex doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, QCortex falls back to the first normalized account ID and `qcortex doctor` warns.
