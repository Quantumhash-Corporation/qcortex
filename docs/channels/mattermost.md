---
summary: "Mattermost bot setup and QCortex config"
read_when:
...
```bash
qcortex plugins install @qcortex/mattermost
```

Local checkout (when running from a git repo):

```bash
qcortex plugins install ./extensions/mattermost
```
...
4. Configure QCortex and start the gateway.
...
Native slash commands are opt-in. When enabled, QCortex registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, QCortex derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as QCortex.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to QCortex.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from QCortex, not `404`.
...
- Approve via:
  - `qcortex pairing list mattermost`
  - `qcortex pairing approve mattermost <CODE>`
...
Use these target formats with `qcortex message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, QCortex derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and QCortex run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="qcortex-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"qcortex-mattermost-interactions",
...
`qcortex message send` and cron/webhook deliveries.
