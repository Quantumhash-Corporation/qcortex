...
```bash
qcortex plugins install @qcortex/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
qcortex plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "QCortex" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `qcortex pairing list nextcloud-talk`
  - `qcortex pairing approve nextcloud-talk <CODE>`
