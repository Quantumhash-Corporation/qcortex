...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects QCortex to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
qcortex plugins install @qcortex/feishu
```

Local checkout (when running from a git repo):

```bash
qcortex plugins install ./extensions/feishu
```
...
If you just installed QCortex, run the wizard:
...
2. Configuring app credentials in QCortex
...
1. You already ran `qcortex channels add` for Feishu
2. The gateway is running (`qcortex gateway status`)
...
```bash
qcortex pairing approve feishu <CODE>
```
...
2. Run `qcortex logs --follow` and look for `chat_id`
...
2. Run `qcortex logs --follow` and look for `open_id`
...
| `qcortex gateway restart` | Restart gateway service       |
| `qcortex logs --follow`   | Tail gateway logs             |
...
4. Check logs: `qcortex logs --follow`
...
5. Ensure the gateway is running: `qcortex gateway status`
6. Check logs: `qcortex logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.qcortex/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.qcortex/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
