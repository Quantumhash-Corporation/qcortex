...
Matrix is an open, decentralized messaging protocol. QCortex connects as a Matrix **user**
...
```bash
qcortex plugins install @qcortex/matrix
```

Local checkout (when running from a git repo):

```bash
qcortex plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `qcortex plugins install @qcortex/matrix`
   - From a local checkout: `qcortex plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: QCortex calls the same
     login endpoint, stores the access token in `~/.qcortex/credentials/matrix/credentials.json`,
...
- On first connection, QCortex requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  QCortex logs a warning.
...
`~/.qcortex/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `qcortex pairing list matrix`
  - `qcortex pairing approve matrix <CODE>`
...
- On startup, QCortex resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
