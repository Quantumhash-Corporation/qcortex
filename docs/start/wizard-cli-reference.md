...
read_when:
  - You need detailed behavior for qcortex onboard
...
This page is the full reference for `qcortex onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.qcortex/qcortex.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `qcortex doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `qcortex pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `qcortex health`.
    - `qcortex status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.qcortex/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.qcortex/agents/<agentId>/agent/auth-profiles.json`
...
`~/.qcortex/credentials/oauth.json` (or `$QCORTEX_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`qcortex onboard`](/cli/onboard)
