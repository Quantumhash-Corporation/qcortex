...
The onboarding wizard is the **recommended** way to set up QCortex on macOS,
...
`qcortex dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`qcortex configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.qcortex/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `qcortex doctor` first.
</Note>
...
Use `qcortex agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.qcortex/workspace-<agentId>`.
...
- CLI command reference: [`qcortex onboard`](/cli/onboard)
