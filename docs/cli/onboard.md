---
summary: "CLI reference for `qcortex onboard` (interactive onboarding wizard)"
read_when:
...
```bash
qcortex onboard
qcortex onboard --flow quickstart
qcortex onboard --flow manual
qcortex onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`QCORTEX_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `qcortex dashboard` (Control UI, no channel setup).
