---
summary: "CLI reference for `qcortex configure` (interactive configuration prompts)"
read_when:
...
Tip: `qcortex config` without a subcommand opens the same wizard. Use
`qcortex config get|set|unset` for non-interactive edits.
...
```bash
qcortex configure
qcortex configure --section model --section channels
```
