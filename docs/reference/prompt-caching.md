...
- With Anthropic API-key auth profiles, QCortex seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, QCortex injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
QCortex exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.qcortex/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$QCORTEX_STATE_DIR/logs/cache-trace.jsonl`
...
- `QCORTEX_CACHE_TRACE=1` enables cache tracing.
- `QCORTEX_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `QCORTEX_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `QCORTEX_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `QCORTEX_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
