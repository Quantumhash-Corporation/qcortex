...
QCortex integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- QCortex owns lifecycle when `channels.signal.autoStart=true`.
...
- QCortex spawns `imsg rpc` as a child process (legacy iMessage integration).
