---
summary: "How QCortex builds prompt context and reports token usage + costs"
read_when:
...
QCortex tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

QCortex assembles its own system prompt on every run. It includes:
...
For images, QCortex downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from QCortex session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `qcortex status --usage` and `qcortex channels list` show
...
`cacheWrite`. If pricing is missing, QCortex shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. QCortex can
...
Anthropic's 1M context window is currently beta-gated. QCortex can inject the
...
QCortex skips the `context-1m-*` beta header because Anthropic currently
