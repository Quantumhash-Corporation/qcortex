...
QCortex features that can generate provider usage or paid API calls.
...
- `qcortex status --usage` and `qcortex channels list` show provider **usage windows**
...
QCortex can pick up credentials from:
...
- `qcortex status --usage`
- `qcortex models status --json`
...
`qcortex models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
