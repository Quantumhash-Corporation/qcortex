...
This is the full reference for the `qcortex onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.qcortex/credentials/oauth.json`; auth profiles live in `~/.qcortex/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.qcortex/credentials/oauth.json` (or `$QCORTEX_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.qcortex/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `qcortex pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `qcortex configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `qcortex health`.
    - Tip: `qcortex status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export QCORTEX_GATEWAY_TOKEN="your-token"
qcortex onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env QCORTEX_GATEWAY_TOKEN
```
...
- Stores it under `~/.qcortex/tools/signal-cli/<version>/`.
...
Typical fields in `~/.qcortex/qcortex.json`:
...
`qcortex agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.qcortex/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.qcortex/agents/<agentId>/sessions/`.
