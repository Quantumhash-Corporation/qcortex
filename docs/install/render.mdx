...
Deploy QCortex on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/qcortex/qcortex)
...
repository configures everything needed to run QCortex:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: QCORTEX_STATE_DIR
        value: /data/.qcortex
      - key: QCORTEX_WORKSPACE_DIR
        value: /data/workspace
      - key: QCORTEX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/qcortex`.
...
If you use the original QCortex repository, Render will not auto-deploy your QCortex. To update it, run a manual Blueprint sync from the dashboard.
...
For QCortex, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any QCortex host.
