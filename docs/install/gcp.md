---
summary: "Run QCortex Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want QCortex running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# QCortex on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent QCortex Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "QCortex 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
```bash
gcloud projects create my-qcortex-project --name="QCortex Gateway"
gcloud config set project my-qcortex-project
```
...
```bash
gcloud compute instances create qcortex-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh qcortex-gateway --zone=us-central1-a
```
...
## 6) Clone the QCortex repository
...
QCORTEX_CONFIG_DIR=/home/$USER/.qcortex
QCORTEX_WORKSPACE_DIR=/home/$USER/.qcortex/workspace
...
When binding to LAN (`QCORTEX_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

```bash
docker compose run --rm qcortex-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
```
...
```bash
gcloud compute ssh qcortex-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop qcortex-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type qcortex-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start qcortex-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create qcortex-deploy \
     --display-name="QCortex Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-qcortex-project \
     --member="serviceAccount:qcortex-deploy@my-qcortex-project.iam.gserviceaccount.com" \
