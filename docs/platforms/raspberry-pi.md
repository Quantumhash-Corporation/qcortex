---
summary: "QCortex on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up QCortex on a Raspberry Pi
  - Running QCortex on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# QCortex on Raspberry Pi

## Goal

Run a persistent, always-on QCortex Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
qcortex config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/qcortex-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/qcortex-compile-cache
mkdir -p /var/tmp/qcortex-compile-cache
export QCORTEX_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `QCORTEX_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running QCortex, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit qcortex
```

```ini
[Service]
Environment=QCORTEX_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/qcortex-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep QCortex state/cache on SSD-backed storage to avoid SD-card
...
Most QCortex features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled qcortex

# Enable if not
sudo systemctl enable qcortex

# Start on boot
sudo systemctl start qcortex
```
...
```bash
# Check logs
journalctl -u qcortex --no-pager -n 100

# Common fix: rebuild
cd ~/qcortex  # if using hackable install
npm run build
sudo systemctl restart qcortex
```
