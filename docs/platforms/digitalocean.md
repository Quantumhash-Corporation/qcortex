---
summary: "QCortex on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up QCortex on DigitalOcean
  - Looking for cheap VPS hosting for QCortex
title: "DigitalOcean"
---

# QCortex on DigitalOcean

## Goal

Run a persistent QCortex Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status qcortex-gateway.service

# View logs
journalctl --user -u qcortex-gateway.service -f
```
...
```bash
qcortex channels login whatsapp
# Scan QR code
```
...
- `~/.qcortex/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u qcortex --no-pager -n 50
```
