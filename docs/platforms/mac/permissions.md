...
- Same path: run the app from a fixed location (for QCortex, `dist/QCortex.app`).
...
```bash
sudo tccutil reset Accessibility ai.qcortex.mac
sudo tccutil reset ScreenCapture ai.qcortex.mac
...
Workaround: move files into the QCortex workspace (`~/.qcortex/workspace`) if you want to avoid per-folder grants.
