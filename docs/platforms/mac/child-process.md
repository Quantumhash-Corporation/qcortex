...
service via the external `qcortex` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.qcortex.gateway`
  (or `ai.qcortex.<profile>` when using `--profile`/`QCORTEX_PROFILE`; legacy `com.qcortex.*` is supported).
