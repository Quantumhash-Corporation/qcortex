---
summary: "VPS hosting hub for QCortex (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/qcortex-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `QCORTEX_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `QCORTEX_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/qcortex-compile-cache`
