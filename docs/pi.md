...
summary: "Architecture of QCortex's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in QCortex
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how QCortex integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

QCortex uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, QCortex directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in QCortex's local TUI mode)                                              |
...
├── pi-tools.ts                    # createQCortexCodingTools()
...
├── qcortex-tools.ts              # QCortex-specific tools
...
Image injection is prompt-local: QCortex loads image refs from the current prompt and
...
2. **Custom Replacements**: QCortex replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **QCortex Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures QCortex's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, QCortex CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
QCortex wraps this with `guardSessionManager()` for tool result safety.
...
QCortex maintains an auth profile store with multiple API keys per provider:
...
QCortex loads custom pi extensions for specialized behavior:
...
QCortex also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | QCortex Embedded                                                                             |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                               |
| Tools           | Default coding tools    | Custom QCortex tool suite                                                                    |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                  |
| Session storage | `~/.pi/agent/sessions/` | `~/.qcortex/agents/<agentId>/sessions/` (or `$QCORTEX_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `QCORTEX_LIVE_TEST=1`)
