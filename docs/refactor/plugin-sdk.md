...
- Publish as `qcortex/plugin-sdk` (or export from core under `qcortex/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `QCortexPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: QCortexConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: QCortexConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: QCortexConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: QCortexConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: QCortexConfig): string;
  };
};
```
...
- Introduce `qcortex/plugin-sdk`.
- Add `api.runtime` to `QCortexPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `qcortexRuntime: ">=2026.2.0"`).
