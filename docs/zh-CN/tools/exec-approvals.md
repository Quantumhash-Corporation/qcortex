...
- **gateway 主机** → gateway 机器上的 `qcortex` 进程
...
如果节点尚未通告执行审批，请直接编辑其本地的 `~/.qcortex/exec-approvals.json`。

CLI：`qcortex approvals` 支持 gateway 或 node 编辑（参见 [Approvals CLI](/cli/approvals)）。
