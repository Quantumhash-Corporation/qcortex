...
- `~/Library/Application Support/QCortex/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `qcortex-canvas://<session>/<path>`

示例：

- `qcortex-canvas://main/` → `<canvasRoot>/main/index.html`
- `qcortex-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `qcortex-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
qcortex nodes canvas present --node <id>
qcortex nodes canvas navigate --node <id> --url "/"
qcortex nodes canvas eval --node <id> --js "document.title"
qcortex nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__qcortex__/a2ui/
```
...
qcortex nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
qcortex nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "qcortex://agent?message=Review%20this%20design";
```
