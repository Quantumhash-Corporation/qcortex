...
你可以在 `~/.qcortex/qcortex.json` 中覆盖此设置：
...
所有日志配置都在 `~/.qcortex/qcortex.json` 的 `logging` 下。
...
- QCortex 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `qcortex plugins enable diagnostics-otel` 启用插件。
...
- `qcortex.tokens`（计数器，属性：`qcortex.token`、`qcortex.channel`、`qcortex.provider`、`qcortex.model`）
- `qcortex.cost.usd`（计数器，属性：`qcortex.channel`、`qcortex.provider`、`qcortex.model`）
- `qcortex.run.duration_ms`（直方图，属性：`qcortex.channel`、`qcortex.provider`、`qcortex.model`）
- `qcortex.context.tokens`（直方图，属性：`qcortex.context`、`qcortex.channel`、`qcortex.provider`、`qcortex.model`）

消息流：

- `qcortex.webhook.received`（计数器，属性：`qcortex.channel`、`qcortex.webhook`）
- `qcortex.webhook.error`（计数器，属性：`qcortex.channel`、`qcortex.webhook`）
- `qcortex.webhook.duration_ms`（直方图，属性：`qcortex.channel`、`qcortex.webhook`）
- `qcortex.message.queued`（计数器，属性：`qcortex.channel`、`qcortex.source`）
- `qcortex.message.processed`（计数器，属性：`qcortex.channel`、`qcortex.outcome`）
- `qcortex.message.duration_ms`（直方图，属性：`qcortex.channel`、`qcortex.outcome`）

队列 + 会话：

- `qcortex.queue.lane.enqueue`（计数器，属性：`qcortex.lane`）
- `qcortex.queue.lane.dequeue`（计数器，属性：`qcortex.lane`）
- `qcortex.queue.depth`（直方图，属性：`qcortex.lane` 或 `qcortex.channel=heartbeat`）
- `qcortex.queue.wait_ms`（直方图，属性：`qcortex.lane`）
- `qcortex.session.state`（计数器，属性：`qcortex.state`、`qcortex.reason`）
- `qcortex.session.stuck`（计数器，属性：`qcortex.state`）
- `qcortex.session.stuck_age_ms`（直方图，属性：`qcortex.state`）
- `qcortex.run.attempt`（计数器，属性：`qcortex.attempt`）
...
- `qcortex.webhook.error`
  - `qcortex.channel`、`qcortex.webhook`、`qcortex.chatId`、`qcortex.error`
- `qcortex.message.processed`
  - `qcortex.channel`、`qcortex.outcome`、`qcortex.chatId`、`qcortex.messageId`、`qcortex.sessionKey`、`qcortex.sessionId`、`qcortex.reason`
- `qcortex.session.stuck`
  - `qcortex.state`、`qcortex.ageMs`、`qcortex.queueDepth`、`qcortex.sessionKey`、`qcortex.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `qcortex doctor`。
