...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `qcortex-e2e-${name}-`));
  const configDir = path.join(homeDir, ".qcortex");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          QCORTEX_SKIP_PROVIDERS: "1",
          QCORTEX_SKIP_GMAIL_WATCHER: "1",
          QCORTEX_SKIP_CRON: "1",
          QCORTEX_SKIP_BROWSER_CONTROL_SERVER: "1",
          QCORTEX_SKIP_CANVAS_HOST: "1",
          QCORTEX_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
