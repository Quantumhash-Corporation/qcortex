...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked QCORTEX_HOME.
  delete process.env.QCORTEX_HOME;
  process.env.QCORTEX_STATE_DIR = path.join(base, ".qcortex");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "qcortex-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".qcortex", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
