  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.QCORTEX_HOME;
        delete env.QCORTEX_STATE_DIR;
        delete env.QCORTEX_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "qcortex.mjs");
