...
import {
  ...
  type WizardPrompter,
} from "qcortex/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: QCORTEX_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: QCortexConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var QCORTEX_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.QCORTEX_TWITCH_ACCESS_TOKEN?.trim();
