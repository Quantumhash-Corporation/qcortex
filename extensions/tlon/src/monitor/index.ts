import type { RuntimeEnv, ReplyPayload, QCortexConfig } from "qcortex/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "qcortex/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as QCortexConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in QCortex config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your QCortex config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.qcortex.ai/concepts/session#secure-dm-mode`;
