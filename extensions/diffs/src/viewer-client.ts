...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-qcortex-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-qcortex-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.qcortexDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.qcortexDiffsError = "true";
