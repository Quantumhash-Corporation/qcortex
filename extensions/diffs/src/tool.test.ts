...
import type { QCortexPluginApi } from "qcortex/plugin-sdk/diffs";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-diffs-tool-"));
...
function createApi(): QCortexPluginApi {
  return {
    id: "diffs",
    name: "Diffs",
    description: "Diffs",
    ...
    runtime: {} as QCortexPluginApi["runtime"],
    logger: {
      info() {},
      warn() {},
      error() {},
