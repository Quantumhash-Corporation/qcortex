...
Read-only diff viewer plugin for **QCortex** agents.
...
Set plugin-wide defaults in `~/.qcortex/qcortex.json`:
...
Before:
QCortex supports plugins.

After:
QCortex supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/qcortex-diffs`).
