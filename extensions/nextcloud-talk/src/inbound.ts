import {
  ...
  type QCortexConfig,
  type RuntimeEnv,
} from "qcortex/plugin-sdk/nextcloud-talk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as QCortexConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as QCortexConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as QCortexConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as QCortexConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as QCortexConfig);
