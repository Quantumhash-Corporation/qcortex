...
import type { QCortexPluginApi, QCortexPluginService } from "qcortex/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: QCortexPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: QCortexPluginApi["config"]): string[] {
...
export default function register(api: QCortexPluginApi) {
  ...
  const timerService: QCortexPluginService = {
