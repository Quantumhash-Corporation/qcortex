# Google Gemini CLI Auth (QCortex plugin)
...
```bash
qcortex plugins enable google-gemini-cli-auth
```
...
```bash
qcortex models auth login --provider google-gemini-cli --set-default
```
...
- `QCORTEX_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `QCORTEX_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
