  ...
  it("uses interactions.callbackBaseUrl when configured", () => {
    const url = resolveInteractionCallbackUrl("default", {
      channels: {
        mattermost: {
          interactions: {
            callbackBaseUrl: "https://gateway.example.com/qcortex",
          },
        },
      },
    });
    expect(url).toBe("https://gateway.example.com/qcortex/mattermost/interactions/default");
  });
...
describe("createMattermostInteractionHandler", () => {
  ...
  it("accepts non-localhost requests when the interaction token is valid", async () => {
    const context = { action_id: "approve", __qcortex_channel_id: "chan-1" };
