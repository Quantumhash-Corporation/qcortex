...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__qcortex_channel_id";
...
type InteractionCallbackConfig = Pick<QCortexConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "qcortex-mattermost-interactions").update(botToken).digest("hex");
}
