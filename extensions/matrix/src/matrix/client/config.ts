import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "qcortex/plugin-sdk/account-id";
import { fetchWithSsrFGuard } from "qcortex/plugin-sdk/matrix";
  ...
  const { response: loginResponse, release: releaseLoginResponse } = await fetchWithSsrFGuard({
    ...
    init: {
      ...
      body: JSON.stringify({
        ...
        initial_device_display_name: resolved.deviceName ?? "QCortex Gateway",
      }),
    },
