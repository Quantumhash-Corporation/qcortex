...
function resolveQCortexRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.QCORTEX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set QCORTEX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveQCortexRoot(), "dist", "extensionAPI.js");
