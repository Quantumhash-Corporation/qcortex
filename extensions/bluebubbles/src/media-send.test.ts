...
import type { QCortexConfig, PluginRuntime } from "qcortex/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): QCortexConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
