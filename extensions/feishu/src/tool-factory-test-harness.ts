import type { AnyAgentTool, QCortexPluginApi } from "qcortex/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: QCortexPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<QCortexPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
