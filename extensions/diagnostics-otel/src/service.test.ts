...
vi.mock("qcortex/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("qcortex/plugin-sdk/diagnostics-otel")>(
    "qcortex/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { QCortexPluginServiceContext } from "qcortex/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "qcortex/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/qcortex-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): QCortexPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(
      telemetryState.histograms.get("qcortex.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("qcortex.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("qcortex.webhook.processed");
    expect(spanNames).toContain("qcortex.message.processed");
    expect(spanNames).toContain("qcortex.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["qcortex.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("qcortex.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "qcortex.reason": expect.stringContaining("…"),
      }),
    );
