...
import type { QCortexPluginApi } from "qcortex/plugin-sdk/device-pair";
import {
...
} from "qcortex/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: QCortexPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.QCORTEX_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: QCortexPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: QCortexPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the QCortex iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the QCortex iOS app:",
            "",
            "```",
            qrAscii,
            "```",
