...
metadata: { "qcortex": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == QCortex `sessions_spawn`
- **File I/O** == QCortex `read`/`write`
- **Remote fetch** == QCortex `web_fetch` (or `exec` with curl when POST is required)
