...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["qcortexPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["qcortex", "pollId"]) ??
    readNestedString(value, ["qcortex", "poll", "id"]) ??
    readNestedString(value, ["data", "qcortexPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "qcortex", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          qcortexPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "qcortex poll vote",
          displayText: "Vote recorded",
          value: { qcortexPollId: pollId, pollId },
        },
      },
    ],
  };
