...
import type { PluginRuntime } from "qcortex/plugin-sdk/nostr";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.QCORTEX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-nostr-"));
  process.env.QCORTEX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.QCORTEX_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".qcortex");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.QCORTEX_STATE_DIR = previous;
    }
