...
`qcortex.invoke --each`) without adding new QCortex code per workflow.
...
This extension depends on QCortex internal modules (the embedded agent runner).
It is intended to ship as a **bundled** QCortex extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.qcortex/extensions` as a standalone plugin directory.
