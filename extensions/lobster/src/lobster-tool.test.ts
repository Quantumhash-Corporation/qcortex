...
import type { QCortexPluginApi, QCortexPluginToolContext } from "qcortex/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<QCortexPluginApi> = {}): QCortexPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<QCortexPluginToolContext> = {}): QCortexPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: QCortexPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
