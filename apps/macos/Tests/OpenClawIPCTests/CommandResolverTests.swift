...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersQCortexBinary() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let qcortexPath = tmp.appendingPathComponent("node_modules/.bin/qcortex")
        try makeExecutableForTests(at: qcortexPath)

        let cmd = CommandResolver.qcortexCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([qcortexPath.path, "gateway"]))
    }
    ...
    @Test func prefersQCortexBinaryOverPnpm() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let qcortexPath = binDir.appendingPathComponent("qcortex")
        ...
        #expect(cmd.prefix(2).elementsEqual([qcortexPath.path, "rpc"]))
    }

    @Test func usesQCortexBinaryWithoutNodeRuntime() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func fallsBackToPnpm() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "qcortex", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "qcortex", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("qcortex@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/qcortex", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "qcortex@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/qcortex'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("qcortex"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == qcortexPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
