...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: QCortexProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: QCortexProtocol.AnyCodable] = [
            "presence": .init([QCortexProtocol.AnyCodable(entry)]),
        ]
        let payload = QCortexProtocol.AnyCodable(payloadMap)
