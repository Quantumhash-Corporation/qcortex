...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["QCORTEX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/qcortex")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("qcortex-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("qcortex-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/qcortex/ (rolling or stdout), or nil if none exist.
