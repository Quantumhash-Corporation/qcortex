...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.qcortex.mac`) even if the shipped
    ...
    {
        if environment["QCORTEX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "qcortex.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "qcortex.nixMode") { return true }
