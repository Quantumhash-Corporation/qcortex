...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: QCortexProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.qcortex", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: QCortexKit.AnyCodable]()) {
                $0[$1.key] = QCortexKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: QCortexProtocol.AnyCodable?) -> [String: QCortexProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: QCortexProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: QCortexKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: QCortexProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("qcortex.control.heartbeat")
    static let controlAgentEvent = Notification.Name("qcortex.control.agent")
}
