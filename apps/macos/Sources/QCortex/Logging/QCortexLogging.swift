...
enum QCortexLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = QCortexOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = QCortexFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        QCortexLogging.bootstrapIfNeeded()
        let label = QCortexLogging.makeLabel(subsystem: subsystem, category: category)
...
struct QCortexOSLogHandler: AppLogLevelBackedHandler {
...
struct QCortexFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = QCortexLogging.parseLabel(self.label)
