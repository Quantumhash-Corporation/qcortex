...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.qcortex/qcortex.json. Prefer QCORTEX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.qcortex/qcortex.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.qcortex/qcortex.json. Restart the gateway to apply."
        }
