...
enum DebugActions {
    private static let verboseDefaultsKey = "qcortex.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "qcortex.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = QCortexPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "QCortex", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = QCortexPaths.configURL
        guard
