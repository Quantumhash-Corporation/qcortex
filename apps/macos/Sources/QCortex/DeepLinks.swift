...
private let deepLinkLogger = Logger(subsystem: "ai.qcortex", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "QCortex is paused", message: "Unpause QCortex to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run QCortex agent?", message: body) else { return }
        }
