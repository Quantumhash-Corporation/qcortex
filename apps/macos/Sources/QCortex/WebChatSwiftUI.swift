...
private let webChatSwiftLogger = Logger(subsystem: "ai.qcortex", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: QCortexChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> QCortexChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(QCortexChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> QCortexChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                QCortexGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<QCortexChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any QCortexChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = QCortexChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: QCortexChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<QCortexChatView>) -> NSViewController
    {
