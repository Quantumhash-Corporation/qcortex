...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.qcortex", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> QCortexSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: QCortexSessionPreviewEntry,
    ...
    private static func cache(payload: QCortexSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: QCortexChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [QCortexKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [QCortexKit.AnyCodable]) -> [QCortexChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(QCortexChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: QCortexChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: QCortexChatMessage) -> Bool {
    ...
    private static func toolNames(for message: QCortexChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: QCortexChatMessage) -> String? {
