...
enum CommandResolver {
    private static let projectRootDefaultsKey = "qcortex.gatewayProjectRootPath"
    private static let helperName = "qcortex"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let qcortexEntry = root.appendingPathComponent("qcortex.mjs").path
        if FileManager().isReadableFile(atPath: qcortexEntry) { return qcortexEntry }
        let binEntry = root.appendingPathComponent("bin/qcortex.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/qcortex")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let qcortexPaths = self.qcortexManagedPaths(home: home)
        if !qcortexPaths.isEmpty {
            extras.insert(contentsOf: qcortexPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + qcortexPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func qcortexManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".qcortex"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func qcortexExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectQCortexExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("qcortex.mjs").path,
            root.appendingPathComponent("bin/qcortex.js").path,
        ]
    ...
    static func hasAnyQCortexInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.qcortexExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func qcortexNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let qcortexPath = self.projectQCortexExecutable(projectRoot: root) {
            return [qcortexPath, subcommand] + extraArgs
        }
        if let qcortexPath = self.qcortexExecutable(searchPaths: searchPaths) {
            return [qcortexPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "qcortex", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            qcortex entrypoint missing (looked for dist/index.js or qcortex.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real qcortex CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/qcortex"
        ...
        \(cliSection)
        \(projectSection)
        if command -v qcortex >/dev/null 2>&1; then
          CLI="$(command -v qcortex)"
          qcortex \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/qcortex.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/qcortex.mjs"
            node "$PRJ/qcortex.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/qcortex.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/qcortex.js"
            node "$PRJ/bin/qcortex.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent qcortex"
          pnpm --silent qcortex \(quotedArgs);
        else
          echo "qcortex CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? QCortexConfigFile.loadDict()
