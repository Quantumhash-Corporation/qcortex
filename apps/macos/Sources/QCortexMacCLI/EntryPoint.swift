...
struct QCortexMacCLI {
    static func main() async {
        ...
        default:
            fputs("qcortex-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      qcortex-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      qcortex-mac discover [--timeout <ms>] [--json] [--include-local]
      qcortex-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      qcortex-mac connect
      qcortex-mac connect --url ws://127.0.0.1:18789 --json
      qcortex-mac discover --timeout 3000 --json
      qcortex-mac wizard --mode local
    """)
}
