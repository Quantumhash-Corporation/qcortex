...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.qcortex.ios", category: "LiveActivity")
    private var currentActivity: Activity<QCortexActivityAttributes>?
    ...
    func startActivity(agentName: String, sessionKey: String) {
        ...
        let attributes = QCortexActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<QCortexActivityAttributes>.activities
    ...
    private func updateCurrent(state: QCortexActivityAttributes.ContentState) {
    ...
    private func connectingState() -> QCortexActivityAttributes.ContentState {
        QCortexActivityAttributes.ContentState(
    ...
    private func idleState() -> QCortexActivityAttributes.ContentState {
        QCortexActivityAttributes.ContentState(
            statusText: "Idle",
            isIdle: true,
            isDisconnected: false,
    ...
    private func disconnectedState() -> QCortexActivityAttributes.ContentState {
        QCortexActivityAttributes.ContentState(
