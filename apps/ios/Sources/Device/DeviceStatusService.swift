...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> QCortexDeviceStatusPayload {
        ...
        return QCortexDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> QCortexDeviceInfoPayload {
        ...
        return QCortexDeviceInfoPayload(
    ...
    private func batteryStatus() -> QCortexBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: QCortexBatteryState = switch device.batteryState {
        ...
        return QCortexBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> QCortexThermalStatusPayload {
        let state: QCortexThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return QCortexThermalStatusPayload(state: state)
    }

    private func storageStatus() -> QCortexStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return QCortexStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
