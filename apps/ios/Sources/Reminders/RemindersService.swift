...
final class RemindersService: RemindersServicing {
    func list(params: QCortexRemindersListParams) async throws -> QCortexRemindersListPayload {
        ...
        let payload: [QCortexReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return QCortexRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: QCortexRemindersAddParams) async throws -> QCortexRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = QCortexReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return QCortexRemindersAddPayload(reminder: payload)
    }
