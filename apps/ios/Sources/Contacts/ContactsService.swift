...
final class ContactsService: ContactsServicing {
    ...
    func search(params: QCortexContactsSearchParams) async throws -> QCortexContactsSearchPayload {
        ...
        return QCortexContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: QCortexContactsAddParams) async throws -> QCortexContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return QCortexContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return QCortexContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> QCortexContactPayload {
        QCortexContactPayload(
