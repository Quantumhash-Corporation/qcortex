...
final class MotionService: MotionServicing {
    func activities(params: QCortexMotionActivityParams) async throws -> QCortexMotionActivityPayload {
        ...
        let mapped: [QCortexMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return QCortexMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: QCortexPedometerParams) async throws -> QCortexPedometerPayload {
        ...
        let payload: QCortexPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = QCortexPedometerPayload(
