...
struct IOSGatewayChatTransport: QCortexChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.qcortex", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> QCortexChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(QCortexChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> QCortexChatHistoryPayload {
        ...
        return try JSONDecoder().decode(QCortexChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [QCortexChatAttachmentPayload]) async throws -> QCortexChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [QCortexChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(QCortexChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(QCortexGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: QCortexGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: QCortexChatEventPayload.self)
                        {
