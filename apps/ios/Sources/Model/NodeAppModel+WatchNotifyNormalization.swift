...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: QCortexWatchNotifyParams) -> QCortexWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [QCortexWatchAction]?,
        kind: String?,
        promptId: String?) -> [QCortexWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> QCortexWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return QCortexWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                QCortexWatchAction(id: "approve", label: "Approve"),
                QCortexWatchAction(id: "decline", label: "Decline", style: "destructive"),
                QCortexWatchAction(id: "open_phone", label: "Open iPhone"),
                QCortexWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            QCortexWatchAction(id: "done", label: "Done"),
            QCortexWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            QCortexWatchAction(id: "open_phone", label: "Open iPhone"),
            QCortexWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: QCortexWatchRisk?,
        priority: QCortexNotificationPriority?) -> QCortexWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: QCortexNotificationPriority?,
        risk: QCortexWatchRisk?) -> QCortexNotificationPriority?
    {
