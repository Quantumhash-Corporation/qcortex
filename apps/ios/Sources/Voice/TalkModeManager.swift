...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__QCORTEX_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<QCortexTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.qcortex", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = QCortexTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> QCortexTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return QCortexTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> QCortexTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> QCortexTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return QCortexTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> QCortexTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = QCortexTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: QCortexTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: QCortexAgentEventPayload.self
            ) else {
                continue
            }
