...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [QCortexCapability.canvas.rawValue, QCortexCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(QCortexCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(QCortexCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = QCortexLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(QCortexCapability.location.rawValue) }

        caps.append(QCortexCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(QCortexCapability.watch.rawValue)
        }
        caps.append(QCortexCapability.photos.rawValue)
        caps.append(QCortexCapability.contacts.rawValue)
        caps.append(QCortexCapability.calendar.rawValue)
        caps.append(QCortexCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(QCortexCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            QCortexScreenCommand.record.rawValue,
            QCortexSystemCommand.notify.rawValue,
            QCortexChatCommand.push.rawValue,
        ...
        if caps.contains(QCortexCapability.camera.rawValue) {
            commands.append(QCortexCameraCommand.list.rawValue)
            commands.append(QCortexCameraCommand.snap.rawValue)
            commands.append(QCortexCameraCommand.clip.rawValue)
        }
        if caps.contains(QCortexCapability.location.rawValue) {
            commands.append(QCortexLocationCommand.get.rawValue)
        }
        if caps.contains(QCortexCapability.device.rawValue) {
            commands.append(QCortexDeviceCommand.status.rawValue)
            commands.append(QCortexDeviceCommand.info.rawValue)
        }
        if caps.contains(QCortexCapability.watch.rawValue) {
            commands.append(QCortexWatchCommand.status.rawValue)
            commands.append(QCortexWatchCommand.notify.rawValue)
        }
        if caps.contains(QCortexCapability.photos.rawValue) {
            commands.append(QCortexPhotosCommand.latest.rawValue)
        }
        if caps.contains(QCortexCapability.contacts.rawValue) {
            commands.append(QCortexContactsCommand.search.rawValue)
            commands.append(QCortexContactsCommand.add.rawValue)
        }
        if caps.contains(QCortexCapability.calendar.rawValue) {
            commands.append(QCortexCalendarCommand.events.rawValue)
            commands.append(QCortexCalendarCommand.add.rawValue)
        }
        if caps.contains(QCortexCapability.reminders.rawValue) {
            commands.append(QCortexRemindersCommand.list.rawValue)
            commands.append(QCortexRemindersCommand.add.rawValue)
        }
        if caps.contains(QCortexCapability.motion.rawValue) {
            commands.append(QCortexMotionCommand.activity.rawValue)
            commands.append(QCortexMotionCommand.pedometer.rawValue)
        }
