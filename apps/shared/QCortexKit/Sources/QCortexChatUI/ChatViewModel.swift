...
private let chatUILogger = Logger(subsystem: "ai.qcortex", category: "QCortexChatUI")

@MainActor
@Observable
public final class QCortexChatViewModel {
    public private(set) var messages: [QCortexChatMessage] = []
    ...
    public var attachments: [QCortexPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [QCortexChatPendingToolCall] = []
    public private(set) var sessions: [QCortexChatSessionEntry] = []
    private let transport: any QCortexChatTransport
    ...
    private var pendingToolCallsById: [String: QCortexChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any QCortexChatTransport) {
    ...
    public var sessionChoices: [QCortexChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [QCortexChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: QCortexPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [QCortexChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: QCortexChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: QCortexChatMessage) -> QCortexChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> QCortexChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return QCortexChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: QCortexChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [QCortexChatMessage],
        incoming: [QCortexChatMessage]) -> [QCortexChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [QCortexChatMessage]) -> [QCortexChatMessage] {
        var result: [QCortexChatMessage] = []
    ...
    private static func dedupeKey(for message: QCortexChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [QCortexChatMessageContent] = [
            QCortexChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> QCortexChatAttachmentPayload in
            QCortexChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> QCortexChatSessionEntry {
        QCortexChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: QCortexChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: QCortexChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: QCortexAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = QCortexChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> QCortexPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
