...
struct ChatMessageBubble: View {
    let message: QCortexChatMessage
    let style: QCortexChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? QCortexChatTheme.userText : QCortexChatTheme.assistantText
    ...
    private var inlineAttachments: [QCortexChatMessageContent] {
    ...
    private var toolCalls: [QCortexChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [QCortexChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? QCortexChatTheme.userBubble
        }
        if self.style == .onboarding {
            return QCortexChatTheme.onboardingAssistantBubble
        }
        return QCortexChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return QCortexChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: QCortexChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? QCortexChatTheme.userText : QCortexChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: QCortexChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(QCortexChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(QCortexChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [QCortexChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: QCortexChatTheme.assistantText)
            }
        }
    }
}
