...
@MainActor
struct QCortexChatComposer: View {
    @Bindable var viewModel: QCortexChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(QCortexChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(QCortexChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \QCortexPendingAttachment.id)
                { (att: QCortexPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            QCortexPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(QCortexChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(QCortexChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(QCortexChatTheme.subtleCard)
        .clipShape(Capsule())
    }
