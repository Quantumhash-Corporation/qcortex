...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> QCortexChatHistoryPayload
{
    QCortexChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> QCortexChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [QCortexChatHistoryPayload],
    sessionsResponses: [QCortexChatSessionsListResponse] = []) async -> (TestChatTransport, QCortexChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { QCortexChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: QCortexChatViewModel,
...
private func sendUserMessage(_ vm: QCortexChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, QCortexChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [QCortexChatHistoryPayload]
    private let sessionsResponses: [QCortexChatSessionsListResponse]

    private let stream: AsyncStream<QCortexChatTransportEvent>
    private let continuation: AsyncStream<QCortexChatTransportEvent>.Continuation

    init(
        historyResponses: [QCortexChatHistoryPayload],
        sessionsResponses: [QCortexChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<QCortexChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> QCortexChatHistoryPayload {
        ...
        return self.historyResponses.last ?? QCortexChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [QCortexChatAttachmentPayload]) async throws -> QCortexChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return QCortexChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> QCortexChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? QCortexChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: QCortexChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = QCortexChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = QCortexChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { QCortexChatViewModel(sessionKey: "main", transport: transport) }
