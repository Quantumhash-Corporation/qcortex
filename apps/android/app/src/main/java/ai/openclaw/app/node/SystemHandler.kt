...
private const val NOTIFICATION_CHANNEL_BASE_ID = "qcortex.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "QCortex Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "QCortex Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "QCortex Active")
      }
