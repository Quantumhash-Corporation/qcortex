...
val androidStoreFile = providers.gradleProperty("QCORTEX_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("QCORTEX_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("QCORTEX_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("QCORTEX_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set QCORTEX_ANDROID_STORE_FILE, " +
            "QCORTEX_ANDROID_STORE_PASSWORD, QCORTEX_ANDROID_KEY_ALIAS, and " +
            "QCORTEX_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/QCortexKit/Sources/QCortexKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.qcortex.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "qcortex-$versionName-$buildType.apk"
