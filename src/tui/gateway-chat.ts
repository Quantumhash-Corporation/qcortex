...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set QCORTEX_GATEWAY_TOKEN/QCORTEX_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.QCORTEX_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.QCORTEX_GATEWAY_PASSWORD);
