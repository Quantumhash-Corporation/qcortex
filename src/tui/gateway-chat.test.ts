...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["QCORTEX_GATEWAY_TOKEN", "QCORTEX_GATEWAY_PASSWORD"]);
  ...
  it.each([
    ...
    const result = await withEnvAsync({ QCORTEX_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("uses QCORTEX_GATEWAY_TOKEN for local mode", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    await withEnvAsync({ QCORTEX_GATEWAY_TOKEN: "env-token" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("prefers QCORTEX_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "QCORTEX_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-tui-file-secret-"));
  ...
  it("resolves only token SecretRef when gateway.auth.mode is token", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-tui-mode-token-"));
  ...
  it("resolves only password SecretRef when gateway.auth.mode is password", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-tui-mode-password-"));
    const tokenMarker = path.join(tempDir, "token-provider-ran");
    const passwordMarker = path.join(tempDir, "password-provider-ran");
    const tokenExecProgram = [
      "const fs=require('node:fs');",
