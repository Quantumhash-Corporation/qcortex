...
export class MemoryIndexManager extends MemoryManagerEmbeddingOps implements MemorySearchManager {
  ...
  protected readonly cfg: QCortexConfig;
  ...
  private async runSyncWithReadonlyRecovery(params?: {
    ...
    try {
      ...
      try {
        ...
        // Log but continue - we need to reopen the database
        log.warn("Failed to close database during readonly recovery", { error: closeErr });
      }
      this.db = this.openDatabase();
    ...
    if (!allowedWorkspace && this.settings.extraPaths.length > 0) {
      ...
      for (const additionalPath of additionalPaths) {
        try {
          ...
          // Path may not exist or be inaccessible - continue checking other paths
        }
      }
    }
  ...
  async close(): Promise<void> {
    ...
    if (pendingSync) {
      try {
        ...
        // Log but continue - we need to close the database anyway
        log.warn("Failed to complete pending sync during close", { error: syncErr });
      }
    }
    this.db.close();
