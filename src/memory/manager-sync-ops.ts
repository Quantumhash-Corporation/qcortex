...
export abstract class MemoryManagerSyncOps {
  protected abstract readonly cfg: QCortexConfig;
  ...
  private seedEmbeddingCache(sourceDb: DatabaseSync): void {
    ...
    try {
      ...
      try {
        ...
        // Log but continue - original error takes precedence
  ...
  private async syncMemoryFiles(params: {
    ...
    for (const stale of staleRows) {
      ...
      try {
        ...
        // Vector table may not exist - continue with cleanup
      }
      ...
      if (this.fts.enabled && this.fts.available) {
        try {
          ...
          // FTS table may not exist - continue with cleanup
        }
      }
    }
  }
    ...
    try {
      if (needsFullReindex) {
        if (
          ...
          process.env.QCORTEX_TEST_MEMORY_UNSAFE_REINDEX === "1"
        ) {
  ...
  private resetIndex() {
    ...
    if (this.fts.enabled && this.fts.available) {
      try {
        ...
        // FTS table may not exist - continue with reset
      }
    }
    this.dropVectorTable();
    this.vector.dims = undefined;
