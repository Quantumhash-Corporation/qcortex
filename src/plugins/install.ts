...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing qcortex.extensions; update the plugin package to include qcortex.extensions (for example ["./dist/index.js"]). See https://docs.qcortex.ai/help/troubleshooting#plugin-install-fails-with-missing-qcortex-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_QCORTEX_EXTENSIONS: "missing_qcortex_extensions",
  EMPTY_QCORTEX_EXTENSIONS: "empty_qcortex_extensions",
...
function ensureQCortexExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_QCORTEX_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json qcortex.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_QCORTEX_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureQCortexExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from qcortex.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-qcortex"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "qcortex security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "qcortex security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "qcortex-npm-pack-",
    spec,
    timeoutMs,
