...
function createSinglePluginWithEmptySlotsConfig(): QCortexConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): QCortexConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "qcortex-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-qcortex-extensions/my-plugin",
      },
      extensionsDir,
    });
