...
import { discoverQCortexPlugins } from "./discovery.js";
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const entry of pluginSdkScopedAliasEntries) {
    ...
    if (resolved) {
      aliasMap[`qcortex/plugin-sdk/${entry.subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: QCortexPluginDefinition;
  register?: QCortexPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as QCortexPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as QCortexPluginDefinition;
...
export function loadQCortexPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverQCortexPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "qcortex/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: QCortexPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as QCortexPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
