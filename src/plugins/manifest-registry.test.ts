...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `qcortex-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "qcortex.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "qcortex.plugin.json");
  const linkedManifest = path.join(rootDir, "qcortex.plugin.json");
