...
describe("web processMessage inbound contract", () => {
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-process-message-"));
  ...
  it("does not force an [qcortex] response prefix in self-chats when identity is unset", async () => {
    capturedDispatchParams = undefined;
