...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "qcortex-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    QCORTEX_GATEWAY_TOKEN: "test-token",
    QCORTEX_CONFIG_DIR: join(sandbox.rootDir, "config"),
    QCORTEX_WORKSPACE_DIR: join(sandbox.rootDir, "qcortex"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      QCORTEX_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      QCORTEX_EXTRA_MOUNTS: undefined,
      QCORTEX_HOME_VOLUME: "qcortex-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("QCORTEX_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("QCORTEX_EXTRA_MOUNTS=");
    expect(envFile).toContain("QCORTEX_HOME_VOLUME=qcortex-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("qcortex-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("qcortex-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg QCORTEX_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm qcortex-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm qcortex-cli config set gateway.mode local");
    expect(log).toContain("run --rm qcortex-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      QCORTEX_CONFIG_DIR: configDir,
      QCORTEX_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when QCORTEX_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      join(configDir, "qcortex.json"),
      JSON.stringify({ gateway: { auth: { mode: "token", token: "config-token-123" } } }),
    );

    const result = runDockerSetup(activeSandbox, {
      QCORTEX_GATEWAY_TOKEN: undefined,
      QCORTEX_CONFIG_DIR: configDir,
      QCORTEX_WORKSPACE_DIR: workspaceDir,
    });
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("QCORTEX_GATEWAY_TOKEN=config-token-123");
  });

  it("treats QCORTEX_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("QCORTEX_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg QCORTEX_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg QCORTEX_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      QCORTEX_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker qcortex-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        QCORTEX_SANDBOX: "1",
        QCORTEX_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            line.includes("compose") && line.includes(" up -d") && line.includes("qcortex-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps qcortex-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate qcortex-gateway"));
  ...
  it("rejects injected multiline QCORTEX_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      QCORTEX_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("QCORTEX_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid QCORTEX_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      QCORTEX_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid QCORTEX_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      QCORTEX_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("QCORTEX_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:qcortex-gateway"');
