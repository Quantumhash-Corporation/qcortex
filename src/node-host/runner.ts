...
function ensureNodePathEnv(): string {
  ensureQCortexCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.QCORTEX_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.QCORTEX_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
