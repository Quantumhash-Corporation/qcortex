...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/qcortex-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
const DEFAULT_TELEGRAM_TEST_CONFIG: QCortexConfig = {
  agents: {
    defaults: {
