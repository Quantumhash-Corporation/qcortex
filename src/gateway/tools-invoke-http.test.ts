...
let lastCreateQCortexToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/qcortex-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateQCortexToolsContext?.agentTo,
          agentThreadId: lastCreateQCortexToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createQCortexTools: (ctx: Record<string, unknown>) => {
      lastCreateQCortexToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateQCortexToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateQCortexToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-qcortex-message-to": "channel:24514",
        "x-qcortex-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
