...
import { resolveQCortexAgentDir } from "../agents/agent-paths.js";
...
import { ensureQCortexModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, QCortexConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.QCORTEX_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.QCORTEX_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.QCORTEX_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.QCORTEX_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.QCORTEX_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.QCORTEX_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.QCORTEX_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<QCortexConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.QCORTEX_CONFIG_PATH,
    token: process.env.QCORTEX_GATEWAY_TOKEN,
    skipChannels: process.env.QCORTEX_SKIP_CHANNELS,
    skipGmail: process.env.QCORTEX_SKIP_GMAIL_WATCHER,
    skipCron: process.env.QCORTEX_SKIP_CRON,
    skipCanvas: process.env.QCORTEX_SKIP_CANVAS_HOST,
    agentDir: process.env.QCORTEX_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.QCORTEX_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.QCORTEX_SKIP_CHANNELS = "1";
  process.env.QCORTEX_SKIP_GMAIL_WATCHER = "1";
  process.env.QCORTEX_SKIP_CRON = "1";
  process.env.QCORTEX_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.QCORTEX_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveQCortexAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-live-state-"));
  process.env.QCORTEX_STATE_DIR = tempStateDir;
  ...
  process.env.QCORTEX_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.qcortex-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveQCortexAgentDir();
  const sanitizedCfg: QCortexConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-live-"));
  const tempConfigPath = path.join(tempDir, "qcortex.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.QCORTEX_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "QCortex live tool probe (local, safe): " +
    ...
    process.env.QCORTEX_CONFIG_PATH = previous.configPath;
    process.env.QCORTEX_GATEWAY_TOKEN = previous.token;
    process.env.QCORTEX_SKIP_CHANNELS = previous.skipChannels;
    process.env.QCORTEX_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.QCORTEX_SKIP_CRON = previous.skipCron;
    process.env.QCORTEX_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.QCORTEX_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.QCORTEX_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureQCortexModelsJson(cfg);
      ...
      const rawModels = process.env.QCORTEX_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via QCORTEX_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.qcortex-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
