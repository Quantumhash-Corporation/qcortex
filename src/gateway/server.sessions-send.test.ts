...
const { createQCortexTools } = await import("../agents/qcortex-tools.js");
...
type SessionSendTool = ReturnType<typeof createQCortexTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createQCortexTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["QCORTEX_GATEWAY_PORT", "QCORTEX_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.QCORTEX_GATEWAY_PORT = String(gatewayPort);
  process.env.QCORTEX_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("QCORTEX_CONFIG_PATH missing in gateway test environment");
      }
