...
describe("live tool probe utils", () => {
  ...
  it("detects anthropic nonce refusal phrasing", () => {
    expect(
      isLikelyToolNonceRefusal(
        "Same request, same answer — this isn't a real QCortex probe. No part of the system asks me to parrot back nonce values.",
      ),
    ).toBe(true);
  });
  ...
  it("retries anthropic nonce refusal output", () => {
    expect(
      shouldRetryToolReadProbe({
        text: "This isn't a real QCortex probe; I won't parrot back nonce values.",
        nonceA: "nonce-a",
        nonceB: "nonce-b",
        provider: "anthropic",
        attempt: 0,
