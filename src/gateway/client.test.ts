...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("qcortex doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with QCORTEX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with QCORTEX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.QCORTEX_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://qcortex-gateway.ai:18789",
      onConnectError,
    });
