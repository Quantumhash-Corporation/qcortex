...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or QCORTEX_GATEWAY_TOKEN)",
    );
  }
  ...
  if (auth.mode === "trusted-proxy") {
    ...
    // Warn if allowUsers is not configured - any authenticated user from proxy can access
    const allowUsers = auth.trustedProxy.allowUsers ?? [];
    if (allowUsers.length === 0) {
      // eslint-disable-next-line no-console
      console.warn(
        "WARNING: gateway.auth.trustedProxy.allowUsers is empty. " +
          "Any user authenticated by your reverse proxy can access the Gateway. " +
          'Consider setting allowUsers to restrict access (e.g., gateway.auth.trustedProxy.allowUsers = ["user@example.com"]).',
      );
    }
  }
}
