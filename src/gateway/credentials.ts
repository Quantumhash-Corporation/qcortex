  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set QCORTEX_GATEWAY_TOKEN/QCORTEX_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.QCORTEX_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.QCORTEX_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
