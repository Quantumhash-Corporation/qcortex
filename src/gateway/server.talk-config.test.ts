...
const TALK_CONFIG_DEVICE_PATH = path.join(
  os.tmpdir(),
  `qcortex-talk-config-device-${process.pid}.json`,
);
...
describe("gateway talk.config", () => {
  it("returns redacted talk config for read scope", async () => {
    ...
    await withServer(async (ws) => {
      ...
      expect(res.payload?.config?.talk?.providers?.elevenlabs?.apiKey).toBe("__QCORTEX_REDACTED__");
      expect(res.payload?.config?.talk?.voiceId).toBe("voice-123");
      expect(res.payload?.config?.talk?.apiKey).toBe("__QCORTEX_REDACTED__");
    });
  });
