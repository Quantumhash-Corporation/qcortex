...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "QCORTEX_STATE_DIR",
  "QCORTEX_CONFIG_PATH",
  "QCORTEX_SKIP_BROWSER_CONTROL_SERVER",
  "QCORTEX_SKIP_GMAIL_WATCHER",
  "QCORTEX_SKIP_CANVAS_HOST",
  "QCORTEX_BUNDLED_PLUGINS_DIR",
  "QCORTEX_SKIP_CHANNELS",
  "QCORTEX_SKIP_PROVIDERS",
  "QCORTEX_SKIP_CRON",
  "QCORTEX_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.QCORTEX_STATE_DIR = path.join(tempHome, ".qcortex");
  delete process.env.QCORTEX_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.QCORTEX_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.QCORTEX_SKIP_GMAIL_WATCHER = "1";
  process.env.QCORTEX_SKIP_CANVAS_HOST = "1";
  process.env.QCORTEX_SKIP_CHANNELS = "1";
  process.env.QCORTEX_SKIP_PROVIDERS = "1";
  process.env.QCORTEX_SKIP_CRON = "1";
  process.env.QCORTEX_TEST_MINIMAL_GATEWAY = "1";
  process.env.QCORTEX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "qcortex-test-no-bundled-extensions")
    : "qcortex-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".qcortex-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".qcortex-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__qcortexTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__qcortexTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["QCORTEX_GATEWAY_TOKEN"]);
  const prev = process.env.QCORTEX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.QCORTEX_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.QCORTEX_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
