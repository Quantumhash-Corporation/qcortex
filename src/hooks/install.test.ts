...
const fixtureRoot = path.join(os.tmpdir(), `qcortex-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@qcortex/test-hooks",
      version: "0.0.1",
      qcortex: { hooks: params.hooks },
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"qcortex":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expectPathInstallFailureContains(result, "qcortex.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expectPathInstallFailureContains(
      result,
      "qcortex.hooks entry resolves outside package directory",
    );
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@qcortex/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@qcortex/test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@qcortex/test-hooks@0.0.2-beta.1",
      name: "@qcortex/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@qcortex/test-hooks@beta"');
    }
  });
});
