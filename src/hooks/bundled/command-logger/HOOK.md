...
homepage: https://docs.qcortex.ai/automation/hooks#command-logger
metadata:
  {
    "qcortex":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.qcortex/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.qcortex/logs/commands.log ~/.qcortex/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/qcortex`:
   ```
   /home/username/.qcortex/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
