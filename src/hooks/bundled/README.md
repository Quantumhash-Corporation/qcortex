...
This directory contains hooks that ship with QCortex. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.qcortex/qcortex.json`:
...
2. Restart gateway: `pkill -9 -f 'qcortex.*gateway' && pnpm qcortex gateway`
3. Enable the hook: `qcortex hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.qcortex.ai/automation/hooks
