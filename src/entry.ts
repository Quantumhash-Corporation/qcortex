...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "qcortex.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "qcortex.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.QCORTEX_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.QCORTEX_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.QCORTEX_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.QCORTEX_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[qcortex] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[qcortex] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[qcortex] ${parsed.error}`);
      process.exit(2);
    }
