...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      QCORTEX_GATEWAY_PORT: String(params.port),
      ...(params.token ? { QCORTEX_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.QCORTEX_STATE_DIR = "/tmp/qcortex-cli-state";
    process.env.QCORTEX_CONFIG_PATH = "/tmp/qcortex-cli-state/qcortex.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        QCORTEX_PROFILE: "dev",
        QCORTEX_STATE_DIR: "/tmp/qcortex-daemon-state",
        QCORTEX_CONFIG_PATH: "/tmp/qcortex-daemon-state/qcortex.json",
        QCORTEX_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.qcortex.gateway.plist",
    });
