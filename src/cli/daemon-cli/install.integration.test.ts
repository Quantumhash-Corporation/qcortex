...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("qcortex-daemon-install-int-");
    configPath = path.join(tempHome, "qcortex.json");
    process.env.HOME = tempHome;
    process.env.QCORTEX_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists and persists the same token used for install env", async () => {
    ...
    expect(installEnv?.QCORTEX_GATEWAY_TOKEN).toBe(persistedToken);
  });
});
