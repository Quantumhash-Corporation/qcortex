...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    QCORTEX_STATE_DIR: "/tmp/qcortex-daemon",
    QCORTEX_CONFIG_PATH: "/tmp/qcortex-daemon/qcortex.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.QCORTEX_STATE_DIR ?? "/tmp/qcortex-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.QCORTEX_CONFIG_PATH ?? `${stateDir}/qcortex.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/qcortex-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.QCORTEX_STATE_DIR = "/tmp/qcortex-cli";
    process.env.QCORTEX_CONFIG_PATH = "/tmp/qcortex-cli/qcortex.json";
