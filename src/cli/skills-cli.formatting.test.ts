...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "qcortex-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "qcortex-bundled-skills-test-"));
    process.env.QCORTEX_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
