...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@qcortex/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@qcortex/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@qcortex/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@qcortex/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @qcortex/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@qcortex/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@qcortex/plugin-alpha@1.2.3",
        installPath: "/tmp/qcortex/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@qcortex/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @qcortex/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@qcortex/plugin-alpha@latest",
      true,
      "/tmp/qcortex/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@qcortex/plugin-alpha@latest",
      installPath: "/tmp/qcortex/extensions/alpha",
      version: "1.2.3",
