...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update QCortex and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["qcortex update", "Update a source checkout (git)"],
        ["qcortex update --channel beta", "Switch to beta channel (git + npm)"],
        ["qcortex update --channel dev", "Switch to dev channel (git + npm)"],
        ["qcortex update --tag beta", "One-off update to a dist-tag or version"],
        ["qcortex update --dry-run", "Preview actions without changing anything"],
        ["qcortex update --no-restart", "Update without restarting the service"],
        ["qcortex update --json", "Output result as JSON"],
        ["qcortex update --yes", "Non-interactive (accept downgrade prompts)"],
        ["qcortex update wizard", "Interactive update wizard"],
        ["qcortex --update", "Shorthand for qcortex update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run qcortex update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.qcortex.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["qcortex update status", "Show channel + version status."],
          ["qcortex update status --json", "JSON output."],
          ["qcortex update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.qcortex.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
