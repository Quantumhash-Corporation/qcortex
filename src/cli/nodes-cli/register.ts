    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["qcortex nodes status", "List known nodes with live status."],
          ["qcortex nodes pairing pending", "Show pending node pairing requests."],
          ['qcortex nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["qcortex nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.qcortex.ai/cli/nodes")}\n`,
    );
