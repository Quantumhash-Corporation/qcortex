...
import type { QCortexConfig, ConfigFileSnapshot } from "../config/types.qcortex.js";
...
vi.mock("../infra/qcortex-root.js", () => ({
  resolveQCortexPackageRoot: vi.fn(),
}));
...
const { resolveQCortexPackageRoot } = await import("../infra/qcortex-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/qcortex-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as QCortexConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/qcortex-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveQCortexPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("qcortex-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveQCortexPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("qcortex");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/qcortex-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "qcortex-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("QCortex update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as QCortexConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveQCortexPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("qcortex-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ QCORTEX_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.QCORTEX_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("qcortex-update-wizard");
    await withEnvAsync({ QCORTEX_GIT_DIR: tempDir }, async () => {
      setTty(true);
