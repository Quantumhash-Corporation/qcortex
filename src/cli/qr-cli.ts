...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary = typeof env.QCORTEX_GATEWAY_TOKEN === "string" ? env.QCORTEX_GATEWAY_TOKEN : "";
...
function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary =
    typeof env.QCORTEX_GATEWAY_PASSWORD === "string" ? env.QCORTEX_GATEWAY_PASSWORD : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy =
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.qcortex.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the QCortex iOS app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("qcortex devices list")}`,
          `  ${theme.command("qcortex devices approve <requestId>")}`,
        );
