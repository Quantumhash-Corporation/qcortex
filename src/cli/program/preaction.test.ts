...
beforeEach(() => {
  ...
  originalHideBanner = process.env.QCORTEX_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.QCORTEX_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.QCORTEX_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("qcortex");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "qcortex", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("qcortex-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "qcortex", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "qcortex", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.QCORTEX_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "qcortex", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "qcortex", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "qcortex", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "qcortex", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "qcortex", "--profile", "work", "config", "validate"],
    });
