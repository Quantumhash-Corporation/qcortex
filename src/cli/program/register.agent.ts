...
${formatHelpExamples([
  ['qcortex agent --to +15555550123 --message "status update"', "Start a new session."],
  ['qcortex agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'qcortex agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'qcortex agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['qcortex agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'qcortex agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.qcortex.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.qcortex.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "qcortex agents set-identity --identity-file ~/.qcortex/workspace/IDENTITY.md --agent main",
