    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "qcortex node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["qcortex node status", "Check node host service status."],
          ["qcortex node install", "Install the node host service."],
          ["qcortex node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.qcortex.ai/cli/node")}\n`,
    );
