...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("qcortex node install"), formatCliCommand("qcortex node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("qcortex node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    QCORTEX_LOG_PREFIX: baseEnv.QCORTEX_LOG_PREFIX ?? "node",
