...
const RELAY_TOKEN_CONTEXT = "qcortex-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const QCORTEX_RELAY_BROWSER = "QCortex/extension-relay";
...
async function resolveGatewayAuthToken(): Promise<string | null> {
  const envToken =
    process.env.QCORTEX_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
    ...
    if (tokenRef) {
      ...
      throw new SecretRefUnavailableError(
        `extension relay requires a resolved gateway token, but gateway.auth.token SecretRef is unavailable (${refLabel}). Set QCORTEX_GATEWAY_TOKEN or resolve your secret provider.`,
      );
    }
...
export async function resolveRelayAcceptedTokensForPort(port: number): Promise<string[]> {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or QCORTEX_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedQCortexRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === QCORTEX_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
