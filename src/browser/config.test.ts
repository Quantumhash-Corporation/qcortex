...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("qcortex");
    expect(profile?.driver).toBe("qcortex");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const qcortex = resolveProfile(resolved, "qcortex");
    expect(qcortex?.driver).toBe("qcortex");
    expect(qcortex?.cdpPort).toBe(18800);
    expect(qcortex?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from QCORTEX_GATEWAY_PORT when unset", () => {
    withEnv({ QCORTEX_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const qcortex = resolveProfile(resolved, "qcortex");
      expect(qcortex?.cdpPort).toBe(19012);
      expect(qcortex?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ QCORTEX_GATEWAY_PORT: undefined }, () => {
      ...
      const qcortex = resolveProfile(resolved, "qcortex");
      expect(qcortex?.cdpPort).toBe(19022);
      expect(qcortex?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(qcortex?.cdpPort).toBe(19000);
    expect(qcortex?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "qcortex");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        qcortex: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("qcortex");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to qcortex profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps qcortex default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("qcortex");
    ...
    it("keeps qcortex default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("qcortex");
    ...
    it("keeps qcortex default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
