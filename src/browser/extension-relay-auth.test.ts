...
import {
  probeAuthenticatedQCortexRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedQCortexRelay({
    baseUrl,
    relayAuthHeader: "x-qcortex-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated qcortex relay probe responses", async () => {
