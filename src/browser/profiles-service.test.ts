...
vi.mock("./chrome.js", () => ({
  resolveQCortexUserDataDir: vi.fn(() => "/tmp/qcortex-test/qcortex/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveQCortexUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "qcortex-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveQCortexUserDataDir).mockReturnValue(userDataDir);
