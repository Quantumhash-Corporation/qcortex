...
export function resolveQCortexUserDataDir(profileName = DEFAULT_QCORTEX_BROWSER_PROFILE_NAME) {
...
export async function launchQCortexChrome(
  ...
  const userDataDir = resolveQCortexUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_QCORTEX_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateQCortexProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 qcortex browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`qcortex browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`qcortex browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 qcortex browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopQCortexChrome(
