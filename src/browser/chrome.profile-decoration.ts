...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".qcortex-profile-decorated");
}
 ...
 */
export function decorateQCortexProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_QCORTEX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_QCORTEX_BROWSER_COLOR).toUpperCase();
