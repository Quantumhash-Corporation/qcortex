import type { BrowserConfig, BrowserProfileConfig, QCortexConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_QCORTEX_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_QCORTEX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "qcortex" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_QCORTEX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_QCORTEX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an QCortex browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for qcortex/qcortex CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: QCortexConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_QCORTEX_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_QCORTEX_BROWSER_PROFILE_NAME]
        ? DEFAULT_QCORTEX_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "qcortex";
