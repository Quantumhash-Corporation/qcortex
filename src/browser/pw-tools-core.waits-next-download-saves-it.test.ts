...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredQCortexTmpDir: vi.fn(() => "/tmp/qcortex"),
}));
vi.mock("../infra/tmp-qcortex-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredQCortexTmpDir.mockReturnValue("/tmp/qcortex");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".qcortex-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredQCortexTmpDir.mockReturnValue("/tmp/qcortex-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "qcortex-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "qcortex-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredQCortexTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "qcortex-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "qcortex-preferred", "downloads")}${path.sep}`),
    );
  });
