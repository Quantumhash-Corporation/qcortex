...
function localQCortexProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "qcortex",
...
function createLocalQCortexResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localQCortexProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveQCortexUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "qcortex-reset-"));
    const profileDir = path.join(tempRoot, "qcortex");
    ...
    const ops = createLocalQCortexResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveQCortexUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "qcortex-reset-no-own-"));
    const profileDir = path.join(tempRoot, "qcortex");
    fs.mkdirSync(profileDir, { recursive: true });
