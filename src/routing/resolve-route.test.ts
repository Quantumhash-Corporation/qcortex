...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: QCortexConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: QCortexConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/qcortex-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<QCortexConfig["bindings"]>[number];
