...
function listAgents(cfg: QCortexConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: QCortexConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<QCortexConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: QCortexConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: QCortexConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: QCortexConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<QCortexConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  QCortexConfig,
  {
    bindingsRef: QCortexConfig["bindings"];
    agentsRef: QCortexConfig["agents"];
    sessionRef: QCortexConfig["session"];
...
function resolveRouteCacheForConfig(cfg: QCortexConfig): Map<string, ResolvedAgentRoute> {
