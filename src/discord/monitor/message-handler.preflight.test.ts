...
type DiscordConfig = NonNullable<
  import("../../config/config.js").QCortexConfig["channels"]
>["discord"];
...
function createPreflightArgs(params: {
  cfg: import("../../config/config.js").QCortexConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("drops bound-thread bot system messages to prevent ACP self-loop", async () => {
    ...
    const result = await preflightDiscordMessage({
      ...
      } as import("../../config/config.js").QCortexConfig,
      discordConfig: {
        allowBots: true,
      } as NonNullable<import("../../config/config.js").QCortexConfig["channels"]>["discord"],
      accountId: "default",
      token: "token",
      runtime: {} as import("../../runtime.js").RuntimeEnv,
      botUserId: "qcortex-bot",
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = {
      ...
      content: "hi <@qcortex-bot>",
      timestamp: new Date().toISOString(),
      channelId,
      attachments: [],
      mentionedUsers: [{ id: "qcortex-bot" }],
      mentionedRoles: [],
      mentionedEveryone: false,
      author: {
        id: "relay-bot-1",
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey qcortex");
