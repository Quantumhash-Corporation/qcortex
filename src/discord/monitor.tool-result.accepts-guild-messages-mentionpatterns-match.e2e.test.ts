  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/qcortex",
      },
    },
    session: { store: "/tmp/qcortex-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bqcortex\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "qcortex: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "QCortex" },
          },
        },
        eventPatch: {
