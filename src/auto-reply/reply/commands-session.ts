...
import { scheduleGatewaySigusr1Restart, triggerQCortexRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting QCortex in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerQCortexRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting QCortex via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
