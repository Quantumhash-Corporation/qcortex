...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop qcortex!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@qcortex_bot", { botUsername: "qcortex_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@qcortex_bot", { botUsername: "qcortex_bot" })).toBe(true);
