...
export function isCommandEnabled(cfg: QCortexConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: QCortexConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as QCortexConfig),
...
export function getCommandDetection(_cfg?: QCortexConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: QCortexConfig) {
