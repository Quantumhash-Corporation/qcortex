...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".qcortex");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.QCORTEX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$QCORTEX_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via QCORTEX_STATE_DIR.
