...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.QCORTEX_GATEWAY_PORT).toBe("18789");
    expect(env.QCORTEX_GATEWAY_TOKEN).toBe("secret");
    expect(env.QCORTEX_SERVICE_MARKER).toBe("qcortex");
    expect(env.QCORTEX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.QCORTEX_SERVICE_VERSION).toBe("string");
    expect(env.QCORTEX_SYSTEMD_UNIT).toBe("qcortex-gateway.service");
    expect(env.QCORTEX_WINDOWS_TASK_NAME).toBe("QCortex Gateway");
    if (process.platform === "darwin") {
      expect(env.QCORTEX_LAUNCHD_LABEL).toBe("ai.qcortex.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", QCORTEX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.QCORTEX_SYSTEMD_UNIT).toBe("qcortex-gateway-work.service");
    expect(env.QCORTEX_WINDOWS_TASK_NAME).toBe("QCortex Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.QCORTEX_LAUNCHD_LABEL).toBe("ai.qcortex.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through QCORTEX_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", QCORTEX_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.QCORTEX_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to QCORTEX_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.QCORTEX_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers QCORTEX_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        QCORTEX_GATEWAY_TOKEN: "qcortex-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.QCORTEX_GATEWAY_TOKEN).toBe("qcortex-token");
  });

  it("omits QCORTEX_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.QCORTEX_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".qcortex"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", QCORTEX_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".qcortex-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", QCORTEX_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".qcortex"));
  });

  it("uses QCORTEX_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", QCORTEX_STATE_DIR: "/var/lib/qcortex" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/qcortex"));
  });

  it("expands ~ in QCORTEX_STATE_DIR", () => {
    const env = { HOME: "/Users/test", QCORTEX_STATE_DIR: "~/qcortex-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/qcortex-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { QCORTEX_STATE_DIR: "C:\\State\\qcortex" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\qcortex");
  });
});
