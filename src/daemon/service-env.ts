...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.QCORTEX_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    QCORTEX_PROFILE: profile,
    QCORTEX_GATEWAY_PORT: String(port),
    QCORTEX_GATEWAY_TOKEN: token,
    QCORTEX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    QCORTEX_SYSTEMD_UNIT: systemdUnit,
    QCORTEX_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    QCORTEX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    QCORTEX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.QCORTEX_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    QCORTEX_GATEWAY_TOKEN: gatewayToken,
    QCORTEX_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    QCORTEX_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    QCORTEX_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    QCORTEX_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    QCORTEX_LOG_PREFIX: "node",
    QCORTEX_SERVICE_MARKER: NODE_SERVICE_MARKER,
    QCORTEX_SERVICE_KIND: NODE_SERVICE_KIND,
    QCORTEX_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  return {
    ...
    QCORTEX_STATE_DIR: sharedEnv.stateDir,
    QCORTEX_CONFIG_PATH: sharedEnv.configPath,
  };
}
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.QCORTEX_STATE_DIR;
  const configPath = env.QCORTEX_CONFIG_PATH;
