...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when QCORTEX_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".qcortex", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when QCORTEX_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", QCORTEX_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".qcortex-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers QCORTEX_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        QCORTEX_PROFILE: "rescue",
        QCORTEX_STATE_DIR: "C:\\State\\qcortex",
      },
      expected: path.join("C:\\State\\qcortex", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", QCORTEX_PROFILE: "default" },
      expected: path.join("/home/test", ".qcortex", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem QCortex Gateway",
          "cd /d C:\\Projects\\qcortex",
          "set NODE_ENV=production",
          "set QCORTEX_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\qcortex",
          environment: {
            NODE_ENV: "production",
            QCORTEX_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\qcortex\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\qcortex\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\QCortex Share\\node.exe" "\\\\fileserver\\QCortex Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\QCortex Share\\node.exe",
            "\\\\fileserver\\QCortex Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from QCORTEX_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ QCORTEX_STATE_DIR: path.join(tmpDir, "custom-state") }),
