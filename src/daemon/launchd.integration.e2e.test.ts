...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `qcortex-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      QCORTEX_LAUNCHD_LABEL: `ai.qcortex.launchd-int-${testId}`,
      QCORTEX_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
