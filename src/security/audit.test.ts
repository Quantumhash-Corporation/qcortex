...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: QCortexConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: QCortexConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: QCortexConfig) => string[];
  isConfigured?: (account: unknown, cfg: QCortexConfig) => boolean;
  isEnabled?: (account: unknown, cfg: QCortexConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "qcortex.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ QCORTEX_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        qcortex: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.QCORTEX_GATEWAY_TOKEN;
    const prevPassword = process.env.QCORTEX_GATEWAY_PASSWORD;
    delete process.env.QCORTEX_GATEWAY_TOKEN;
    delete process.env.QCORTEX_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.QCORTEX_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.QCORTEX_GATEWAY_PASSWORD;
      } else {
        process.env.QCORTEX_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/qcortex-safe-bins"];
    const cfg: QCortexConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("qcortex-sbx-browser-old\nqcortex-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "qcortex-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "qcortex-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("qcortex-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("qcortex-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "qcortex-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "qcortex-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-qcortex.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: QCortexConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: QCortexConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): QCortexConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: QCortexConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: QCortexConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: QCortexConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain("~/.qcortex/credentials/discord-allowFrom.json:team.owner");
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: QCortexConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.QCORTEX_GATEWAY_TOKEN;
    process.env.QCORTEX_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: QCortexConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<QCortexConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: QCortexConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.qcortex",
      configPath: "/Users/test/Dropbox/.qcortex/qcortex.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: QCortexConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "qcortex.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "qcortex.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@qcortex/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@qcortex/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.QCORTEX_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.QCORTEX_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
