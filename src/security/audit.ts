...
import type { ConfigFileSnapshot, QCortexConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: QCortexConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your QCortex state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your QCortex state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.QCORTEX_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.QCORTEX_GATEWAY_PASSWORD) ||
  ...
  if (auth.mode === "trusted-proxy") {
    ...
    if (allowUsers.length === 0) {
      // Only warn for loopback; require allowUsers when exposed to network
      const severity = bind === "loopback" ? "warn" : "critical";
      findings.push({
        ...
        detail:
          ...
          (bind !== "loopback"
            ? " This is a critical security risk when the gateway is exposed to non-loopback addresses."
            : ""),
        remediation:
          "Set gateway.auth.trustedProxy.allowUsers to restrict access to specific users " +
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("qcortex security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.QCORTEX_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: QCortexConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: QCortexConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: QCortexConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("qcortex status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("qcortex security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set QCORTEX_GATEWAY_TOKEN/QCORTEX_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("qcortex security audit --deep")}".`,
    });
  }
