...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("qcortex-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("qcortex-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("qcortex-lid-a-", (first) => {
      withTempDirSync("qcortex-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.qcortex when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "qcortex-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers QCORTEX_HOME over HOME", () => {
    vi.stubEnv("QCORTEX_HOME", "/srv/qcortex-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/qcortex-home"));
...
describe("shortenHomePath", () => {
  it("uses $QCORTEX_HOME prefix when QCORTEX_HOME is set", () => {
    vi.stubEnv("QCORTEX_HOME", "/srv/qcortex-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/qcortex-home")}/.qcortex/qcortex.json`)).toBe(
      "$QCORTEX_HOME/.qcortex/qcortex.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $QCORTEX_HOME replacement when QCORTEX_HOME is set", () => {
    vi.stubEnv("QCORTEX_HOME", "/srv/qcortex-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/qcortex-home")}/.qcortex/qcortex.json`),
    ).toBe("config: $QCORTEX_HOME/.qcortex/qcortex.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/qcortex")).toBe(path.resolve(os.homedir(), "qcortex"));
  });
  ...
  it("prefers QCORTEX_HOME for tilde expansion", () => {
    vi.stubEnv("QCORTEX_HOME", "/srv/qcortex-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/qcortex")).toBe(path.resolve("/srv/qcortex-home", "qcortex"));
