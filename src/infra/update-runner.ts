...
const DEFAULT_PACKAGE_NAME = "qcortex";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["qcortex", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "qcortex.mjs");
    ...
    const doctorStep = await runStep(
      step("qcortex doctor", doctorArgv, gitRoot, { QCORTEX_UPDATE_IN_PROGRESS: "1" }),
    );
