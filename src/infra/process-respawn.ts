...
import { triggerQCortexRestart } from "./restart.js";
 ...
 * - QCORTEX_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.QCORTEX_NO_RESPAWN)) {
    return { mode: "disabled" };
  }
  const supervisor = detectRespawnSupervisor(process.env);
  if (supervisor) {
    if (supervisor === "launchd" || supervisor === "schtasks") {
      const restart = triggerQCortexRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
