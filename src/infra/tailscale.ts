...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.QCORTEX_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for QCortex. You can keep running the web gateway without it: \`${formatCliCommand("qcortex gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
