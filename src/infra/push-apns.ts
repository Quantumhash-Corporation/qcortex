...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.QCORTEX_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.QCORTEX_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set QCORTEX_APNS_TEAM_ID and QCORTEX_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.QCORTEX_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.QCORTEX_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.QCORTEX_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set QCORTEX_APNS_PRIVATE_KEY_P8 or QCORTEX_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading QCORTEX_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createQCortexPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    qcortex: createQCortexPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
