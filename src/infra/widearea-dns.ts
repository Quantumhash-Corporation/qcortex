...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.QCORTEX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*qcortex-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "qcortex";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "qcortex");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "qcortex-gw");
  ...
  records.push(`_qcortex-gw._tcp IN PTR ${instanceLabel}._qcortex-gw._tcp`);
  records.push(`${instanceLabel}._qcortex-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._qcortex-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; qcortex-content-hash: ${contentHash}\n${contentBody}`;
}
