 ...
 * returning control to the caller (typically `triggerQCortexRestart` →
...
/**
 * Parse qcortex gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("qcortex")) {
 ...
 * Returns only PIDs that belong to qcortex gateway processes (not the current process).
 */
