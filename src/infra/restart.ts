...
export function triggerQCortexRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.QCORTEX_SYSTEMD_UNIT,
      process.env.QCORTEX_PROFILE,
    );
  ...
  const label =
    process.env.QCORTEX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.QCORTEX_PROFILE);
