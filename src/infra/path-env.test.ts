...
let ensureQCortexCliOnPath: typeof import("./path-env.js").ensureQCortexCliOnPath;

describe("ensureQCortexCliOnPath", () => {
  const envKeys = [
    "PATH",
    "QCORTEX_PATH_BOOTSTRAPPED",
    "QCORTEX_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureQCortexCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling qcortex exists", () => {
    const tmp = abs("/tmp/qcortex-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "qcortex");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.QCORTEX_PATH_BOOTSTRAPPED;

    ensureQCortexCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.QCORTEX_PATH_BOOTSTRAPPED = "1";
    ensureQCortexCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/qcortex-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "qcortex");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/qcortex-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "qcortex");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "qcortex");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/qcortex-path/case-linuxbrew");
