...
vi.mock("./qcortex-root.js", () => ({
  resolveQCortexPackageRoot: vi.fn(async () => null),
  resolveQCortexPackageRootSync: vi.fn(() => null),
}));
...
let qcortexRoot: typeof import("./qcortex-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    qcortexRoot = await import("./qcortex-root.js");
  });
  ...
  it("uses resolveQCortexPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/qcortex");
    (
      qcortexRoot.resolveQCortexPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/qcortex"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "qcortex" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "qcortex.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-qcortex");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/qcortex-bundle");
    (
      qcortexRoot.resolveQCortexPackageRootSync as unknown as ReturnType<typeof vi.fn>
