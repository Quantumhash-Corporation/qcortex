...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__qcortex_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "qcortex-root");
...
describe("resolveQCortexPackageRoot", () => {
  let resolveQCortexPackageRoot: typeof import("./qcortex-root.js").resolveQCortexPackageRoot;
  let resolveQCortexPackageRootSync: typeof import("./qcortex-root.js").resolveQCortexPackageRootSync;

  beforeAll(async () => {
    ({ resolveQCortexPackageRoot, resolveQCortexPackageRootSync } =
      await import("./qcortex-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "qcortex");
    const pkgRoot = path.join(project, "node_modules", "qcortex");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "qcortex" }));

    expect(resolveQCortexPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "qcortex");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "qcortex.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "qcortex" }));

    expect(resolveQCortexPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveQCortexPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-qcortex package roots", async () => {
    const pkgRoot = fx("not-qcortex");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-qcortex" }));

    expect(resolveQCortexPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveQCortexPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveQCortexPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
