...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __qcortexDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__qcortexDiagnosticEventsState) {
    globalStore.__qcortexDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__qcortexDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: QCortexConfig): boolean {
