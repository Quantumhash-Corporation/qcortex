...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred QCortex tmp root", async () => {
    const tmpRoot = resolvePreferredQCortexTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideQCortex = path.join(os.tmpdir(), "outside-qcortex", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
