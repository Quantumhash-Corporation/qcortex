...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: QCortexConfig | null = null;
  ...
  get(key: string, cfg: QCortexConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: QCortexConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: QCortexConfig): void {
