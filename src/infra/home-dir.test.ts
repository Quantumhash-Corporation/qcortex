...
describe("resolveEffectiveHomeDir", () => {
  it("prefers QCORTEX_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/qcortex-home"));
  });
  ...
  it("expands QCORTEX_HOME when set to ~", () => {
    const env = {
      QCORTEX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for QCORTEX_HOME", () => {
    const result = resolveRequiredHomeDir(
      { QCORTEX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when QCORTEX_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ QCORTEX_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { QCORTEX_HOME: "/srv/qcortex-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/qcortex-home")}/x`);
  });
