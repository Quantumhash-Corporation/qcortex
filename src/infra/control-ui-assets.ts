...
import { resolveQCortexPackageRoot, resolveQCortexPackageRootSync } from "./qcortex-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveQCortexPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "qcortex" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveQCortexPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
