...
const SYSTEMD_SUPERVISOR_HINT_ENV_VARS = [
  "QCORTEX_SYSTEMD_UNIT",
  "INVOCATION_ID",
  "SYSTEMD_EXEC_PID",
  "JOURNAL_STREAM",
] as const;

const WINDOWS_TASK_SUPERVISOR_HINT_ENV_VARS = ["QCORTEX_WINDOWS_TASK_NAME"] as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "QCORTEX_SERVICE_MARKER",
  "QCORTEX_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.QCORTEX_SERVICE_MARKER?.trim();
    const serviceKind = env.QCORTEX_SERVICE_KIND?.trim();
