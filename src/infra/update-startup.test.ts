...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["QCORTEX_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.QCORTEX_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveQCortexPackageRoot } = await import("./qcortex-root.js"));
    ...
    vi.mocked(resolveQCortexPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveQCortexPackageRoot).mockResolvedValue("/opt/qcortex");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/qcortex/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
