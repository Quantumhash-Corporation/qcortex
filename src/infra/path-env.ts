...
function candidateBinDirs(opts: EnsureQCortexPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `qcortex` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "qcortex");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.QCORTEX_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "qcortex"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `qcortex` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureQCortexCliOnPath(opts: EnsureQCortexPathOpts = {}) {
  if (isTruthyEnvValue(process.env.QCORTEX_PATH_BOOTSTRAPPED)) {
    return;
  }
