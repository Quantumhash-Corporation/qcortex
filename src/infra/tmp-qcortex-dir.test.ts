...
import { POSIX_QCORTEX_TMP_DIR, resolvePreferredQCortexTmpDir } from "./tmp-qcortex-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredQCortexTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `qcortex-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredQCortexTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_QCORTEX_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredQCortexTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredQCortexTmpDir", () => {
  it("prefers /tmp/qcortex when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_QCORTEX_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/qcortex when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_QCORTEX_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/qcortex when /tmp/qcortex is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/qcortex when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/qcortex is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/qcortex is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/qcortex is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback QCortex temp dir/);
  });
