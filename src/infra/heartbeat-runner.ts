...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: QCortexConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: QCortexConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: QCortexConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: QCortexConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: QCortexConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: QCortexConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: QCortexConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: QCortexConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
