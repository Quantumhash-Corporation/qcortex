...
function newToken() {
  const { token } = generatePairingToken(DEFAULT_TOKEN_EXPIRY_MS);
  return token;
}
...
export async function approveNodePairing(
  ...
  return await withLock(async () => {
    ...
    const tokenExpiresAtMs = now + DEFAULT_TOKEN_EXPIRY_MS;
...
export async function verifyNodeToken(
  ...
  return verifyPairingToken(token, node.token, node.tokenExpiresAtMs)
    ? { ok: true, node }
    : { ok: false };
}
