...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses qcortex-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "qcortex-gateway" },
          { pid: process.pid, cmd: "qcortex-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'qcortex'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple qcortex pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "qcortex-gateway" },
          { pid: pid2, cmd: "qcortex-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-qcortex pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks qcortex processes — non-qcortex listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "qcortex-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty qcortex stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty qcortex stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + qcortex pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "qcortex-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'qcortex'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("qcortex")
      // inside the mid-loop flush: a non-qcortex cmd between two entries must not
      // be pushed, but the following qcortex entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-qcortex entry first, then qcortex entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid qcortex entry must still be found.
      const stdout = `p0\ncqcortex-gateway\np${stalePid}\ncqcortex-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero qcortex pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-qcortex stdout (line 145)", () => {
    it("treats status 1 + non-qcortex stdout as port-free (not an qcortex process)", () => {
      // status 1 + non-empty stdout where no qcortex pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill qcortex pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-qcortex output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no qcortex pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
