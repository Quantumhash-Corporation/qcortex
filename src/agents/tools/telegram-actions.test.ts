...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): QCortexConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as QCortexConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): QCortexConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as QCortexConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: QCortexConfig, accountId = "media") {
    await handleTelegramAction(
