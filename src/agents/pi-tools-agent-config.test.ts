...
import { createQCortexCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-pi-tools-"));
    ...
    try {
      ...
      const tools = createQCortexCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: QCortexConfig) {
    return createQCortexCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<QCortexConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<QCortexConfig["agents"]>["list"]>[number]["tools"];
  }): QCortexConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createQCortexCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createQCortexCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createQCortexCodingTools({
      config: cfg,
    ...
    const defaultTools = createQCortexCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createQCortexCodingTools({
      config: cfg,
    ...
    const bobTools = createQCortexCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createQCortexCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createQCortexCodingTools({
      config: cfg,
