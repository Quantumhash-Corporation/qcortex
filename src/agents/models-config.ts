...
type ModelsConfig = NonNullable<QCortexConfig["models"]>;
...
export async function ensureQCortexModelsJson(
  config?: QCortexConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveQCortexAgentDir();
