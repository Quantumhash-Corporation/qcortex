...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "qcortex.sandbox=1");
  args.push("--label", `qcortex.sessionKey=${params.scopeKey}`);
  args.push("--label", `qcortex.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `qcortex.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "qcortex.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`qcortex sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`qcortex sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("qcortex sandbox recreate --all");
}
