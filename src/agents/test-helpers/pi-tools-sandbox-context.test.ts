...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("qcortex-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("qcortex-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("qcortex-sbx-");
