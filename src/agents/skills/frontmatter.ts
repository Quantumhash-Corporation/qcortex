...
import {
  applyQCortexManifestInstallCommonFields,
  getFrontmatterString,
  normalizeStringList,
  parseQCortexManifestInstallBase,
  parseFrontmatterBool,
  resolveQCortexManifestBlock,
  resolveQCortexManifestInstall,
  resolveQCortexManifestOs,
  resolveQCortexManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  QCortexSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseQCortexManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyQCortexManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveQCortexMetadata(
  frontmatter: ParsedSkillFrontmatter,
): QCortexSkillMetadata | undefined {
  const metadataObj = resolveQCortexManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveQCortexManifestRequires(metadataObj);
  const install = resolveQCortexManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveQCortexManifestOs(metadataObj);
  return {
