...
function resolveSkillsLimits(config?: QCortexConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "qcortex-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "qcortex-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveQCortexMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: QCortexConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`qcortex skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
