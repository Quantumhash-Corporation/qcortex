    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "qcortex.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "qcortex.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "qcortex.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("qcortex-");
  const acpxRoot = await tempDirs.make("qcortex-acpx-plugin-");
  const helperRoot = await tempDirs.make("qcortex-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("qcortex-");
  const pluginRoot = await tempDirs.make("qcortex-plugin-");
  const outsideDir = await tempDirs.make("qcortex-outside-");
