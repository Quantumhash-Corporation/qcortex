...
const originalBundledDir = process.env.QCORTEX_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("qcortex-bundled-");
  const workspaceDir = await createTempDir("qcortex-workspace-");
  const pluginRoot = path.join(bundledPluginsDir, "diffs");

  await fs.mkdir(path.join(pluginRoot, "skills", "diffs"), { recursive: true });
  await fs.writeFile(
    path.join(pluginRoot, "qcortex.plugin.json"),
    JSON.stringify(
      {
        id: "diffs",
        skills: ["./skills"],
...
afterEach(async () => {
  process.env.QCORTEX_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.QCORTEX_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
