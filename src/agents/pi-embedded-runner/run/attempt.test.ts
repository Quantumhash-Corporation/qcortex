...
function createOllamaProviderConfig(injectNumCtxForOpenAICompat: boolean): QCortexConfig {
  return {
    models: {
      providers: {
        ollama: {
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    const legacy = buildAfterTurnLegacyCompactionParams({
      attempt: {
        ...
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as QCortexConfig,
