...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["QCORTEX_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-subagent-"));
  ...
  it("uses isolated temp state when QCORTEX_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "qcortex-test-state"));
  });
});
