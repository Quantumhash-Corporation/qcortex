...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses QCORTEX_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      QCORTEX_HOME: "/srv/qcortex-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/qcortex-home"), ".qcortex", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".qcortex", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("qcortex-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-workspace-hardlink-"));
    try {
