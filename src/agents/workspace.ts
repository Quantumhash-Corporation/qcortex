...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.QCORTEX_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".qcortex", `workspace-${profile}`);
  }
  return path.join(home, ".qcortex", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".qcortex";
