...
test("exec sets QCORTEX_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-qcortex-shell", {
    command: "node -e \"process.stdout.write(process.env.QCORTEX_SHELL || '')\"",
    pty: true,
  });
