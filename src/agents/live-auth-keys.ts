...
const GOOGLE_LIVE_SINGLE_KEY = "QCORTEX_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "QCORTEX_LIVE_ANTHROPIC_KEY",
    listVar: "QCORTEX_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "QCORTEX_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `QCORTEX_LIVE_${base}_KEY`;
