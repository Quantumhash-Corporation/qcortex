...
import { resolvePreferredQCortexTmpDir } from "../infra/tmp-qcortex-dir.js";
...
async function withOutsideHardlinkInQCortexTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside qcortex tmp root",
      media: path.join(os.tmpdir(), "outside-qcortex", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked QCortex tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked QCortex tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInQCortexTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked QCortex tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInQCortexTmp(
