...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureQCortexModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): QCortexConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): QCortexConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): QCortexConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-workspace-"));
    const now = Date.now();
