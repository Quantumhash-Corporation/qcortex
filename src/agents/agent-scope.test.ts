...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: QCortexConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/qcortex" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: QCortexConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: QCortexConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: QCortexConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: QCortexConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: QCortexConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: QCortexConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: QCortexConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/qcortex");
  });

  it("uses QCORTEX_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "qcortex-home");
    vi.stubEnv("QCORTEX_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as QCortexConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".qcortex", "workspace"));
  });

  it("uses QCORTEX_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "qcortex-home");
    vi.stubEnv("QCORTEX_HOME", home);
    // Clear state dir so it falls back to QCORTEX_HOME
    vi.stubEnv("QCORTEX_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as QCortexConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".qcortex", "agents", "main", "agent"));
  });
});
