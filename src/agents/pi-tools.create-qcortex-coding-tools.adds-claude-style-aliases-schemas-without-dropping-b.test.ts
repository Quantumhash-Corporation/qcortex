...
const defaultTools = createQCortexCodingTools({ senderIsOwner: true });
...
describe("createQCortexCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: QCortexConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createQCortexCodingTools({
    ...
    const anthropicTools = createQCortexCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: QCortexConfig = {
      tools: {
        exec: {
    ...
    const allowed = createQCortexCodingTools({
    ...
    const denied = createQCortexCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createQCortexCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
