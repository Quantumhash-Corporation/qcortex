...
describe("canvas host state dir defaults", () => {
  it("uses QCORTEX_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("qcortex-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("QCortex Canvas");
      } finally {
        await handler.close();
      }
    });
