...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.qcortexCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.qcortexCanvasA2UIAction &&
        typeof window.qcortexCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.qcortexSendUserAction === "function";

  // Use textContent and create elements programmatically to avoid XSS
  ...
  // Clear and rebuild status element safely
  statusEl.textContent = "Bridge: ";
  statusEl.appendChild(bridgeSpan);

  const iOSSpan = document.createElement("span");
  iOSSpan.textContent = " · iOS=" + (hasIOS() ? "yes" : "no");
  statusEl.appendChild(iOSSpan);

  const androidSpan = document.createElement("span");
  androidSpan.textContent = " · Android=" + (hasAndroid() ? "yes" : "no");
  statusEl.appendChild(androidSpan);

  const onStatus = (ev) => {
    const d = ev && ev.detail || {};
    log("Action status: id=" + (d.id || "?") + " ok=" + String(!!d.ok) + (d.error ? (" error=" + d.error) : ""));
  };
  window.addEventListener("qcortex:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android QCortex node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.qcortexSendUserAction === "function"
        ? window.qcortexSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.QCORTEX_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.QCORTEX_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>QCortex Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
