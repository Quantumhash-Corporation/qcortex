...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "qcortex.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    QCORTEX_PLUGIN_MANIFEST_CACHE_MS: process.env.QCORTEX_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-config-plugin-validation-"));
    ...
    process.env.QCORTEX_STATE_DIR = path.join(suiteHome, ".qcortex");
    process.env.QCORTEX_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.QCORTEX_STATE_DIR === undefined) {
      delete process.env.QCORTEX_STATE_DIR;
    } else {
      process.env.QCORTEX_STATE_DIR = envSnapshot.QCORTEX_STATE_DIR;
    }
    if (envSnapshot.QCORTEX_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.QCORTEX_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.QCORTEX_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.QCORTEX_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
