...
import type { QCortexConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareQCortexVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  qcortex config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  qcortex config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: QCortexConfig,
  pathSegments: string[],
): { changed: boolean; next: QCortexConfig } {
...
function coerceConfig(value: unknown): QCortexConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as QCortexConfig;
}
...
function stampConfigVersion(cfg: QCortexConfig): QCortexConfig {
...
function warnIfConfigFromFuture(cfg: QCortexConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareQCortexVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer QCortex (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as QCortexConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): QCortexConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as QCortexConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: QCortexConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as QCortexConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.QCORTEX_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.QCORTEX_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.QCORTEX_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.QCORTEX_WATCH_SESSION === "string" &&
        deps.env.QCORTEX_WATCH_SESSION.trim().length > 0
          ? deps.env.QCORTEX_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.QCORTEX_WATCH_COMMAND === "string" &&
        deps.env.QCORTEX_WATCH_COMMAND.trim().length > 0
          ? deps.env.QCORTEX_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `QCORTEX_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: QCortexConfig | null = null;
let runtimeConfigSourceSnapshot: QCortexConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.QCORTEX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.QCORTEX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): QCortexConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): QCortexConfig {
