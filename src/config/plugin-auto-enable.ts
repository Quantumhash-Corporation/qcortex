...
function isWhatsAppConfigured(cfg: QCortexConfig): boolean {
...
function isGenericChannelConfigured(cfg: QCortexConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: QCortexConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: QCortexConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: QCortexConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: QCortexConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: QCortexConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: QCortexConfig, pluginId: string): QCortexConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
