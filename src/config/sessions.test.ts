...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ QCORTEX_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-qcortex",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-qcortex#general");
  });
  ...
  it("derives session transcripts dir from QCORTEX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { QCORTEX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when QCORTEX_STATE_DIR differs from stored paths", () => {
