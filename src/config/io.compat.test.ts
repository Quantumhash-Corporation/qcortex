...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.qcortex/qcortex.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".qcortex", 19001);
  ...
  it("defaults to ~/.qcortex/qcortex.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".qcortex", "qcortex.json"));
    });
  });

  it("uses QCORTEX_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { QCORTEX_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".qcortex", "qcortex.json"));
    });
  });

  it("honors explicit QCORTEX_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".qcortex", 20002, "custom.json");
      const io = createIoForHome(home, { QCORTEX_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".qcortex", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "qcortex.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
