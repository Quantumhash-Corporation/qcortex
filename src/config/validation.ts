...
import type { QCortexConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: QCortexConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: QCortexConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: QCortexConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = QCortexSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as QCortexConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as QCortexConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as QCortexConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as QCortexConfig,
  };
}
