...
import type { ConfigFileSnapshot } from "./types.qcortex.js";
import { QCortexSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(QCortexSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.qcortex/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/qcortex/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/qcortex/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/qcortex/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/qcortex/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = QCortexSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "QCortexConfig";
    const hints = mainSchemaHints;
