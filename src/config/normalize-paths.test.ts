  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.qcortex/logs/qcortex.log" },
        hooks: {
          path: "~/.qcortex/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.qcortex/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".qcortex", "logs", "qcortex.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".qcortex", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".qcortex", "telegram.token"),
      );
