...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("qcortex-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "qcortex-sandbox-browser-global" },
      agentBrowser: { network: "qcortex-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("qcortex-sandbox-browser-agent");
  });
