...
import { buildWebSearchProviderConfig, withTempHome, writeQCortexConfig } from "./test-helpers.js";
import { QCortexSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = QCortexSchema.safeParse({
      $schema: "https://qcortex.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://qcortex.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = QCortexSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = QCortexSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeQCortexConfig(home, {
        gateway: { bind: "${QCORTEX_BIND}" },
      });

      const prev = process.env.QCORTEX_BIND;
      process.env.QCORTEX_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.QCORTEX_BIND;
        } else {
          process.env.QCORTEX_BIND = prev;
        }
      }
    });
  });
