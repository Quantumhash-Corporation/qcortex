...
const TEMP_MEDIA_PREFIX = "qcortex-media-";
...
function createGroqAudioConfig(): QCortexConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): QCortexConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): QCortexConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: QCortexConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredQCortexTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        QCORTEX_AGENT_DIR: isolatedAgentDir,
