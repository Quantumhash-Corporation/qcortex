...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        QCORTEX_BASE_ENV: "base",
        QCORTEX_TO_REMOVE: undefined,
      },
      env: {
        QCORTEX_TEST_ENV: "ok",
      },
    ...
    expect(resolved.QCORTEX_BASE_ENV).toBe("base");
    expect(resolved.QCORTEX_TEST_ENV).toBe("ok");
    expect(resolved.QCORTEX_TO_REMOVE).toBeUndefined();
  });
