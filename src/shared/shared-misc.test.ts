...
describe("shared/frontmatter", () => {
  ...
  test("resolveQCortexManifestBlock parses JSON5 metadata and picks qcortex block", () => {
    const frontmatter = {
      metadata: "{ qcortex: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveQCortexManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });
