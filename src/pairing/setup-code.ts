...
function resolveAuth(cfg: QCortexConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.QCORTEX_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayTokenSecretRef(
  ...
  const hasTokenEnvCandidate = Boolean(
    env.QCORTEX_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim(),
  );
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.QCORTEX_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
    );
...
async function resolveGatewayPasswordSecretRef(
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.QCORTEX_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim()) ||
