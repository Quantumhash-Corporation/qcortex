...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".qcortex");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      QCORTEX_STATE_DIR: paths.stateDir,
      QCORTEX_CONFIG_PATH: paths.configPath,
