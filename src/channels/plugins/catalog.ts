...
import { discoverQCortexPlugins } from "../../plugins/discovery.js";
import type { QCortexPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, QCortexPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["QCORTEX_PLUGIN_CATALOG_PATHS", "QCORTEX_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<QCortexPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: QCortexPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: QCortexPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverQCortexPlugins({ workspaceDir: options.workspaceDir });
