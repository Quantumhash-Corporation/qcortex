...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: QCortexConfig, policy: ChannelAccessPolicy) => QCortexConfig;
  resolveAllowlist: (params: { cfg: QCortexConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: QCortexConfig; resolved: TResolved }) => QCortexConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: QCortexConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: QCortexConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: QCortexConfig, policy: ChannelAccessPolicy): QCortexConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: QCortexConfig, policy: ChannelAccessPolicy): QCortexConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: QCortexConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: QCortexConfig; resolved: string[] }) => {
      calls.push("apply");
