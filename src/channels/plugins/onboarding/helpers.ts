...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in QCortex config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "QCORTEX_SECRET",
      envVarFormatError:
        'Use an env var name like "QCORTEX_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
