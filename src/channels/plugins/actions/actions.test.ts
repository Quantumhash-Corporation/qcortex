...
function telegramCfg(): QCortexConfig {
  return { channels: { telegram: { botToken: "tok" } } } as QCortexConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: QCortexConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as QCortexConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as QCortexConfig;
...
function createSignalAccountOverrideCfg(): QCortexConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as QCortexConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as QCortexConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as QCortexConfig,
        accountId: undefined,
        params: {
