...
const { makeStorePath } = createCronStoreHarness({ prefix: "qcortex-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses QCORTEX_HOME for tilde expansion", () => {
    vi.stubEnv("QCORTEX_HOME", "/srv/qcortex-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/qcortex-home", "cron", "jobs.json"));
  });
});
