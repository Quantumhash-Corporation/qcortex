...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "qcortex-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".qcortex", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<QCortexConfig> = {},
): QCortexConfig {
  const base: QCortexConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<QCortexConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<QCortexConfig>,
