...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("qcortex-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.QCORTEX_STATE_DIR).toBe(path.join(tempHome.home, ".qcortex"));
    ...
    expect(process.env.QCORTEX_STATE_DIR).toBe(previousStateDir);
