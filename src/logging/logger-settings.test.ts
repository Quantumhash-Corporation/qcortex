...
let originalQCortexLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.QCORTEX_TEST_FILE_LOG;
  originalQCortexLogLevel = process.env.QCORTEX_LOG_LEVEL;
  delete process.env.QCORTEX_TEST_FILE_LOG;
  delete process.env.QCORTEX_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.QCORTEX_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalQCortexLogLevel === undefined) {
    delete process.env.QCORTEX_LOG_LEVEL;
  } else {
    process.env.QCORTEX_LOG_LEVEL = originalQCortexLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
