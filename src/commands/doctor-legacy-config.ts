...
export function normalizeCompatibilityConfigValues(cfg: QCortexConfig): {
  config: QCortexConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: QCortexConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as QCortexConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as QCortexConfig["browser"],
    };
    changes.push(
