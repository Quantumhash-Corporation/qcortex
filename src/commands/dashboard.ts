...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary = env.QCORTEX_GATEWAY_TOKEN?.trim();
  if (primary) {
    return primary;
  }
...
export async function dashboardCommand(
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set QCORTEX_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `qcortex dashboard`.",
    );
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control QCortex.");
  } else if (hint) {
    runtime.log(hint);
  }
}
