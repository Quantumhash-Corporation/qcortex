...
function resolveAgentName(cfg: QCortexConfig, agentId: string) {
...
function resolveAgentModel(cfg: QCortexConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: QCortexConfig): AgentSummary[] {
