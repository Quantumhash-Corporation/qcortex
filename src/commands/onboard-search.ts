...
function rawKeyValue(config: QCortexConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: QCortexConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: QCortexConfig, provider: SearchProvider): QCortexConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: QCortexConfig, result: QCortexConfig): QCortexConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with qcortex configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — QCortex will store a reference instead of the API key.",
