...
function pruneAuthProfiles(
  cfg: QCortexConfig,
  profileIds: Set<string>,
): { next: QCortexConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("qcortex models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("qcortex models auth setup-token")} or ${formatCliCommand(
      "qcortex configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "qcortex models auth login --provider openai-codex",
    )} or ${formatCliCommand("qcortex configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("qcortex configure")}\` or \`${formatCliCommand("qcortex onboard")}\`.`;
}
