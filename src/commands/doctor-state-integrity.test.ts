...
type EnvSnapshot = {
  HOME?: string;
  QCORTEX_HOME?: string;
  QCORTEX_STATE_DIR?: string;
  QCORTEX_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    QCORTEX_HOME: process.env.QCORTEX_HOME,
    QCORTEX_STATE_DIR: process.env.QCORTEX_STATE_DIR,
    QCORTEX_OAUTH_DIR: process.env.QCORTEX_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: QCortexConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: QCortexConfig) {
...
async function runStateIntegrityText(cfg: QCortexConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "qcortex-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.QCORTEX_HOME = tempHome;
    process.env.QCORTEX_STATE_DIR = path.join(tempHome, ".qcortex");
    delete process.env.QCORTEX_OAUTH_DIR;
    fs.mkdirSync(process.env.QCORTEX_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when QCORTEX_OAUTH_DIR is explicitly configured", async () => {
    process.env.QCORTEX_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: QCortexConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints qcortex-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: QCortexConfig = {};
    ...
    expect(text).toMatch(/qcortex sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/qcortex sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /qcortex sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
