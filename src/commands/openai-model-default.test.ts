...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: QCortexConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: QCortexConfig) => { changed: boolean; next: QCortexConfig };
  defaultModel: string;
  overrideConfig: QCortexConfig;
  alreadyDefaultConfig: QCortexConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: QCortexConfig) => config,
      applyDefaultConfig: (config: QCortexConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: QCortexConfig = { agents: { defaults: {} } };
