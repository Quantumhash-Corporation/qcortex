...
beforeAll(() => {
  envSnapshot = captureEnv(["QCORTEX_PROFILE"]);
  process.env.QCORTEX_PROFILE = "isolated";
});
...
vi.mock("../infra/qcortex-root.js", () => ({
  resolveQCortexPackageRoot: vi.fn().mockResolvedValue("/tmp/qcortex"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/qcortex/pnpm-lock.yaml",
      markerPath: "/tmp/qcortex/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.qcortex.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.qcortex.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("qcortex status --all") ||
          line.includes("qcortex --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("QCORTEX_GATEWAY_TOKEN", "abcd1234", async () => {
