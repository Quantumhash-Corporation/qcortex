...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "QCortex", ".qcortex");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/qcortex-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".qcortex");
  ...
  it("anchors cloud detection to OS homedir when QCORTEX_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".qcortex");
    const originalQCortexHome = process.env.QCORTEX_HOME;
    process.env.QCORTEX_HOME = "/tmp/qcortex-home-override";
    ...
    try {
      ...
      if (originalQCortexHome === undefined) {
        delete process.env.QCORTEX_HOME;
      } else {
        process.env.QCORTEX_HOME = originalQCortexHome;
      }
    }
  });
