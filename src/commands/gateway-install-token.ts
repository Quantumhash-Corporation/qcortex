...
import { readConfigFileSnapshot, writeConfigFile, type QCortexConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("qcortex config set gateway.auth.mode token")} or ${formatCliCommand("qcortex config set gateway.auth.mode password")}.`,
  ].join(" ");
}
...
export async function resolveGatewayInstallToken(
  ...
  const envToken =
    options.env.QCORTEX_GATEWAY_TOKEN?.trim() || options.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
