...
import type { QCortexConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: QCortexConfig): string[] {
...
export async function noteSecurityWarnings(cfg: QCortexConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("qcortex security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.qcortex/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("qcortex approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.qcortex.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("qcortex configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("qcortex config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("qcortex doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("qcortex config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('qcortex config set session.dmScope "per-channel-peer"') +
