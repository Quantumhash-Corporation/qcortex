...
import { resolveQCortexPackageRoot } from "../infra/qcortex-root.js";
...
function resolveMode(cfg: QCortexConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("QCortex doctor");

  const root = await resolveQCortexPackageRoot({
  ...
  let cfg: QCortexConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("qcortex configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("qcortex config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("qcortex setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("qcortex config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("qcortex config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("qcortex doctor --fix")}" to apply changes.`);
  }
