...
type PromptDefaultModelResult = { model?: string; config?: QCortexConfig };
...
function resolveConfiguredModelRaw(cfg: QCortexConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: QCortexConfig): string[] {
...
export function applyPrimaryModel(cfg: QCortexConfig, model: string): QCortexConfig {
...
export function applyModelAllowlist(cfg: QCortexConfig, models: string[]): QCortexConfig {
  const defaults = cfg.agents?.defaults;
