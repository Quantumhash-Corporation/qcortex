...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.QCORTEX_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("qcortex-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "qcortex");
  ...
  it("uses QCORTEX_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.QCORTEX_GATEWAY_TOKEN = envToken;
        ...
        await runNonInteractiveOnboarding(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "QCORTEX_GATEWAY_TOKEN",
          },
          runtime,
        );
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.QCORTEX_STATE_DIR = stateDir;
      process.env.QCORTEX_CONFIG_PATH = path.join(stateDir, "qcortex.json");
