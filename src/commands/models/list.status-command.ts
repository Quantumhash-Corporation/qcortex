...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveQCortexAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("qcortex models auth setup-token")}\` or \`${formatCliCommand("qcortex configure")}\`.`
          : `Run \`${formatCliCommand("qcortex configure")}\` or set an API key env var.`;
