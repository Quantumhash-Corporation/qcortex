...
export async function loadValidConfigOrThrow(): Promise<QCortexConfig> {
...
export async function updateConfig(
  mutator: (cfg: QCortexConfig) => QCortexConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: QCortexConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: QCortexConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("qcortex agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
