...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveQCortexAgentDir: vi.fn().mockReturnValue("/tmp/qcortex-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/qcortex-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/qcortex-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveQCortexAgentDir: mocks.resolveQCortexAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/qcortex-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveQCortexAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/qcortex-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/qcortex-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/qcortex-agent-custom");
