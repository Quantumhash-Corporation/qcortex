...
const SEARCH_TOOL = "https://docs.qcortex.ai/mcp.SearchQCortex";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.qcortex.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('qcortex docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.qcortex.ai/");
      runtime.log(`Search: ${formatCliCommand('qcortex docs "your query"')}`);
    }
    return;
  }
