...
import type { QCortexConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: QCortexConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const token = env.QCORTEX_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  return token || undefined;
}

function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const password = env.QCORTEX_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim();
...
export function buildNetworkHints(cfg: QCortexConfig) {
