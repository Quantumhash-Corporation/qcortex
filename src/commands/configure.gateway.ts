...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "QCORTEX_GATEWAY_TOKEN",
          placeholder: "QCORTEX_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like QCORTEX_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. QCortex will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: QCortex trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.qcortex.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
