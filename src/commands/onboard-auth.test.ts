...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under QCORTEX_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("qcortex-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-oauth-sync-"));
    ...
    process.env.QCORTEX_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-oauth-nosync-"));
    process.env.QCORTEX_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside QCORTEX_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qcortex-oauth-external-"));
    process.env.QCORTEX_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* QCORTEX_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to QCORTEX_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("qcortex-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
