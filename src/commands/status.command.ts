...
export async function statusCommand(
  ...
  runtime.log(theme.heading("QCortex status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`qcortex devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("qcortex devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("qcortex devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("qcortex security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("qcortex security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.qcortex.ai/faq");
  runtime.log("Troubleshooting: https://docs.qcortex.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("qcortex status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("qcortex logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("qcortex status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("qcortex gateway probe")}`);
  }
}
