...
const createTempDir = () => tempDirs.make("qcortex-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets QCORTEX_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "qcortex",
    });

    expect(env.QCORTEX_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("qcortex");
  });

  it("overrides pre-existing QCORTEX_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      QCORTEX_SHELL: "wrong",
    });
    expect(env.QCORTEX_SHELL).toBe("acp-client");
  ...
  it("preserves QCORTEX_SHELL even when stripKeys contains it", () => {
    const env = resolveAcpClientSpawnEnv(
      {
        QCORTEX_SHELL: "skill-overridden",
        OPENAI_API_KEY: "sk-leaked",
      },
      { stripKeys: new Set(["QCORTEX_SHELL", "OPENAI_API_KEY"]) },
    );
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "qcortex", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "qcortex", "dist", "entry.js");
    const shimPath = path.join(dir, "qcortex.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\qcortex\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/qcortex-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/qcortex-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/qcortex-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
