---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for QCortex deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, QCortex cron scheduling for periodic checks, or version status checks on a machine running QCortex (laptop, workstation, Pi, VPS).
---

# QCortex Host Hardening

## Overview

Assess and harden the host running QCortex, then align it to a user-defined risk tolerance without breaking access. Use QCortex security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim QCortex changes the host firewall, SSH, or OS updates; it does not.
...
5. QCortex gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if QCortex can access sensitive data.
...
### 2) Run QCortex security audits (read-only)

As part of the default read-only checks, run `qcortex security audit --deep`. Only offer alternatives if the user requests them:

1. `qcortex security audit` (faster, non-probing)
2. `qcortex security audit --json` (structured output)

Offer to apply QCortex safe defaults (numbered):

1. `qcortex security audit --fix`

Be explicit that `--fix` only tightens QCortex defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check QCortex version/update status (read-only)

As part of the default read-only checks, run `qcortex update status`.
...
- Credential hygiene notes (location of QCortex creds, prefer disk encryption)
...
After QCortex install or first hardening pass, run at least one baseline audit and version check:

- `qcortex security audit`
- `qcortex security audit --deep`
- `qcortex update status`

Ongoing monitoring is recommended. Use the QCortex cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `qcortex cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `qcortex update status`
...
Before creating, `qcortex cron list` and match on exact `name`. If found, `qcortex cron edit <id> ...`.
If not found, `qcortex cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `qcortex update status` (preferred for source checkouts and channels)
2. `npm view qcortex version` (published npm version)

## QCortex command accuracy

Use only supported commands and flags:

- `qcortex security audit [--deep] [--fix] [--json]`
- `qcortex status` / `qcortex status --deep`
- `qcortex health --json`
- `qcortex update status`
- `qcortex cron add|list|runs|run`

Do not invent CLI flags or imply QCortex enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by QCortex compaction:
