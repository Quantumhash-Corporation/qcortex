...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `qcortex/plugin-sdk` alias is a prefix match.
    alias: [
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `qcortex/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "qcortex/plugin-sdk",
  ...
  test: {
    ...
    exclude: [
      ...
      "**/*.live.test.ts",
      "**/*.e2e.test.ts",
      // Known-failing tests (pre-existing issues, need investigation)
      "**/pi-embedded-subscribe*.test.ts",
      "**/pi-embedded-helpers.sanitize-session*.test.ts",
      "**/reply.directive*.test.ts",
