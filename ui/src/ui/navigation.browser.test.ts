...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/qcortex/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/qcortex");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/qcortex/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__QCORTEX_CONTROL_UI_BASE_PATH__ = "/qcortex";
    const app = mountApp("/qcortex/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/qcortex");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/qcortex/sessions");
  });
