...
export async function handleWhatsAppStart(host: QCortexApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: QCortexApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: QCortexApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: QCortexApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: QCortexApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: QCortexApp): string {
...
function resolveGatewayHttpAuthHeader(host: QCortexApp): string | null {
...
function buildGatewayHttpHeaders(host: QCortexApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: QCortexApp) {
...
export function handleNostrProfileToggleAdvanced(host: QCortexApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: QCortexApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: QCortexApp) {
  const state = host.nostrProfileFormState;
