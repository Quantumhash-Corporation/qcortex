        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-daypart-grid">
            ${stats.weekdayTotals.map((part) => {
              ...
              const bg =
                part.tokens > 0
                  ? `rgba(var(--accent-rgb), ${0.12 + intensity * 0.6})`
                  : "transparent";
              return html`
        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-hour-grid">
            ${stats.hourTotals.map((value, hour) => {
              ...
              const bg =
                value > 0 ? `rgba(var(--accent-rgb), ${0.08 + intensity * 0.7})` : "transparent";
              const title = `${hour}:00 · ${formatTokens(value)} tokens`;
              const border =
                intensity > 0.7 ? "rgba(var(--accent-rgb), 0.6)" : "rgba(var(--accent-rgb), 0.2)";
